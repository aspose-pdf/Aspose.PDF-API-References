---
title: FontColor
second_title: Aspose.PDF per .NET API Reference
description: Classe che rappresenta il colore del testo.
type: docs
weight: 2280
url: /it/net/aspose.pdf.facades/fontcolor/
---
## FontColor class

Classe che rappresenta il colore del testo.

```csharp
public sealed class FontColor
```

## Costruttori

| Nome | Descrizione |
| --- | --- |
| [FontColor](fontcolor#constructor)() | Inizializza il colore. |
| [FontColor](fontcolor#constructor_1)(int, int, int) | Inizializza il colore con i componenti di colore specificati. |

## Proprietà

| Nome | Descrizione |
| --- | --- |
| [Blue](../../aspose.pdf.facades/fontcolor/blue) { get; set; } | Componente blu del colore. |
| [Green](../../aspose.pdf.facades/fontcolor/green) { get; set; } | Componente verde del colore. |
| [Red](../../aspose.pdf.facades/fontcolor/red) { get; set; } | Componente rossa del colore. |

### Guarda anche

* spazio dei nomi [Aspose.Pdf.Facades](../../aspose.pdf.facades)
* assemblea [Aspose.PDF](../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.PDF.dll -->
