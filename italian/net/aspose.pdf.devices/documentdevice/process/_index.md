---
title: "DocumentDevice.Process"
second_title: "Aspose.PDF per .NET - Riferimento API"
description: "DocumentDevice metodo. Ogni dispositivo rappresenta qualche operazione sul documento, ad es. possiamo convertire un documento pdf in un altro formato"
type: docs
weight: 10
url: /it/net/aspose.pdf.devices/documentdevice/process/
---
## Process(Document, int, int, Stream) {#process}

Ogni dispositivo rappresenta una certa operazione sul documento, ad es. possiamo convertire il documento pdf in un altro formato.

```csharp
public abstract void Process(Document document, int fromPage, int toPage, Stream output)
```

| Parametro | Tipo | Descrizione |
| --- | --- | --- |
| document | Document | Il documento da elaborare. |
| fromPage | Int32 | Definisce la pagina da cui iniziare l'elaborazione. |
| toPage | Int32 | Definisce l'ultima pagina da elaborare. |
| output | Stream | Definisce lo stream dove sono memorizzati i risultati dell'elaborazione. |

### Vedi anche

* class [Document](../../../aspose.pdf/document/)
* class [DocumentDevice](../)
* namespace [Aspose.Pdf.Devices](../../../aspose.pdf.devices/)
* assembly [Aspose.PDF](../../../)

---

## Process(Document, Stream) {#process_2}

Elabora l'intero documento e salva i risultati nello stream.

```csharp
public void Process(Document document, Stream output)
```

| Parametro | Tipo | Descrizione |
| --- | --- | --- |
| document | Document | Il documento da elaborare. |
| output | Stream | Definisce lo stream dove sono memorizzati i risultati dell'elaborazione. |

### Vedi anche

* class [Document](../../../aspose.pdf/document/)
* class [DocumentDevice](../)
* namespace [Aspose.Pdf.Devices](../../../aspose.pdf.devices/)
* assembly [Aspose.PDF](../../../)

---

## Process(Document, string) {#process_3}

Elabora l'intero documento e salva i risultati nel file.

```csharp
public void Process(Document document, string outputFileName)
```

| Parametro | Tipo | Descrizione |
| --- | --- | --- |
| document | Document | Il documento da elaborare. |
| outputFileName | String | Definisce il file dove sono memorizzati i risultati dell'elaborazione. |

### Vedi anche

* class [Document](../../../aspose.pdf/document/)
* class [DocumentDevice](../)
* namespace [Aspose.Pdf.Devices](../../../aspose.pdf.devices/)
* assembly [Aspose.PDF](../../../)

---

## Process(Document, int, int, string) {#process_1}

Elabora alcune pagine del documento e salva i risultati nel file.

```csharp
public void Process(Document document, int fromPage, int toPage, string outputFileName)
```

| Parametro | Tipo | Descrizione |
| --- | --- | --- |
| document | Document | Il documento da elaborare. |
| fromPage | Int32 | La prima pagina da cui iniziare l'elaborazione. |
| toPage | Int32 | L'ultima pagina dell'elaborazione. |
| outputFileName | String | Definisce il file dove sono memorizzati i risultati dell'elaborazione. |

### Vedi anche

* class [Document](../../../aspose.pdf/document/)
* class [DocumentDevice](../)
* namespace [Aspose.Pdf.Devices](../../../aspose.pdf.devices/)
* assembly [Aspose.PDF](../../../)


