---
title: "BorderInfo.BorderInfo"
second_title: "Référence de l'API Aspose.PDF pour .NET"
description: "Constructeur BorderInfo. Initialise une nouvelle instance de la classe BorderInfo"
type: docs
weight: 10
url: /fr/net/aspose.pdf/borderinfo/borderinfo/
---
## BorderInfo(BorderSide, Color) {#constructor_2}

Initialise une nouvelle instance de la classe [`BorderInfo`](../).

```csharp
public BorderInfo(BorderSide borderSide, Color borderColor)
```

| Paramètre | Type | Description |
| --- | --- | --- |
| borderSide | BorderSide | Indique les informations des côtés de la bordure. Par exemple : (int)(BorderSide.Left &#x7C; BorderSide.Top). |
| borderColor | Color | La couleur de la bordure. |

### Voir aussi

* enum [BorderSide](../../borderside/)
* class [Color](../../color/)
* class [BorderInfo](../)
* namespace [Aspose.Pdf](../../../aspose.pdf/)
* assembly [Aspose.PDF](../../../)

---

## BorderInfo() {#constructor}

Initialise une nouvelle instance de la classe [`BorderInfo`](../).

```csharp
public BorderInfo()
```

### Voir aussi

* class [BorderInfo](../)
* namespace [Aspose.Pdf](../../../aspose.pdf/)
* assembly [Aspose.PDF](../../../)

---

## BorderInfo(BorderSide) {#constructor_1}

Initialise une nouvelle instance de la classe [`BorderInfo`](../).

```csharp
public BorderInfo(BorderSide borderSide)
```

| Paramètre | Type | Description |
| --- | --- | --- |
| borderSide | BorderSide | Indique les informations des côtés de la bordure. Par exemple : (BorderSide.Left &#x7C; BorderSide.Top). |

### Voir aussi

* enum [BorderSide](../../borderside/)
* class [BorderInfo](../)
* namespace [Aspose.Pdf](../../../aspose.pdf/)
* assembly [Aspose.PDF](../../../)

---

## BorderInfo(BorderSide, float) {#constructor_4}

Initialise une nouvelle instance de la classe [`BorderInfo`](../).

```csharp
public BorderInfo(BorderSide borderSide, float borderWidth)
```

| Paramètre | Type | Description |
| --- | --- | --- |
| borderSide | BorderSide | Indique les informations des côtés de la bordure. Par exemple : (BorderSide.Left &#x7C; BorderSide.Top). |
| borderWidth | Single | La largeur de la bordure. |

### Voir aussi

* enum [BorderSide](../../borderside/)
* class [BorderInfo](../)
* namespace [Aspose.Pdf](../../../aspose.pdf/)
* assembly [Aspose.PDF](../../../)

---

## BorderInfo(BorderSide, float, Color) {#constructor_5}

Initialise une nouvelle instance de la classe [`BorderInfo`](../).

```csharp
public BorderInfo(BorderSide borderSide, float borderWidth, Color borderColor)
```

| Paramètre | Type | Description |
| --- | --- | --- |
| borderSide | BorderSide | Indique les informations des côtés de la bordure. Par exemple : (BorderSide.Left &#x7C; BorderSide.Top). |
| borderWidth | Single | La largeur de la bordure. |
| borderColor | Color | La couleur de la bordure. |

### Voir aussi

* enum [BorderSide](../../borderside/)
* class [Color](../../color/)
* class [BorderInfo](../)
* namespace [Aspose.Pdf](../../../aspose.pdf/)
* assembly [Aspose.PDF](../../../)

---

## BorderInfo(BorderSide, GraphInfo) {#constructor_3}

Initialise une nouvelle instance de la classe [`BorderInfo`](../).

```csharp
public BorderInfo(BorderSide borderSide, GraphInfo info)
```

| Paramètre | Type | Description |
| --- | --- | --- |
| borderSide | BorderSide | Indique les informations des côtés de la bordure. Par exemple : (BorderSide.Left &#x7C; BorderSide.Top). |
| info | GraphInfo | Les informations de la bordure. |

### Voir aussi

* enum [BorderSide](../../borderside/)
* class [GraphInfo](../../graphinfo/)
* class [BorderInfo](../)
* namespace [Aspose.Pdf](../../../aspose.pdf/)
* assembly [Aspose.PDF](../../../)


