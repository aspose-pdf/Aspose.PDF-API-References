---
title: Class SaveOptions.BorderPartStyle
second_title: Aspose.PDF for .NET API Reference
description: Classe Aspose.Pdf.SaveOptionsBorderPartStyle. Représente les informations d'une partie de la bordure (haut, bas, côté gauche ou côté droit)
type: docs
weight: 9890
url: /fr/net/aspose.pdf/saveoptions.borderpartstyle/
---
## Classe SaveOptions.BorderPartStyle

Représente les informations d'une partie de la bordure (haut, bas, côté gauche ou côté droit)

```csharp
public class BorderPartStyle
```

## Constructeurs

| Nom | Description |
| --- | --- |
| [BorderPartStyle](../../aspose.pdf/saveoptions.borderpartstyle/.ctor)() | Le constructeur par défaut. |

## Propriétés

| Nom | Description |
| --- | --- |
| [WidthInPoints](../../aspose.pdf/saveoptions.borderpartstyle/widthinpoints) { get; set; } | Représente la largeur de la ligne de bordure en points. Doit être un nombre supérieur à zéro. |

## Champs

| Nom | Description |
| --- | --- |
| [Color](../../aspose.pdf/saveoptions.borderpartstyle/color) | Représente la couleur de la ligne de bordure |
| [LineType](../../aspose.pdf/saveoptions.borderpartstyle/linetype) | Représente le type de ligne de bordure - par exemple, pointillée ou pleine |

### Voir aussi

* classe [SaveOptions](../saveoptions/)
* espace de noms [Aspose.Pdf](../../aspose.pdf/)
* assembly [Aspose.PDF](../../)