---
title: Class ListDataResponseT
second_title: Aspose.PDF for .NET API Reference
description: Classe Aspose.Pdf.AI.ListDataResponse1T. Représente une réponse de données de liste contenant des informations supplémentaires telles que les premiers et derniers ID et s'il y a plus d'éléments.
type: docs
weight: 670
url: /fr/net/aspose.pdf.ai/listdataresponse-1/
---
## Classe ListDataResponse&lt;T&gt;

Représente une réponse de données de liste contenant des informations supplémentaires telles que les premiers et derniers ID et s'il y a plus d'éléments.

```csharp
public class ListDataResponse<T> : DataResponse<T>
```

## Constructeurs

| Nom | Description |
| --- | --- |
| [ListDataResponse](listdataresponse/)() | Le constructeur par défaut. |

## Propriétés

| Nom | Description |
| --- | --- |
| [Data](../../aspose.pdf.ai/dataresponse-1/data/) { get; set; } |  |
| [Detail](../../aspose.pdf.ai/baseresponse/detail/) { get; set; } | Obtient ou définit le détail de la réponse. |
| [Error](../../aspose.pdf.ai/baseresponse/error/) { get; set; } | Obtient ou définit l'erreur de réponse HTTP. |
| [ErrorMessage](../../aspose.pdf.ai/baseresponse/errormessage/) { get; } | Obtient ou définit les informations sur l'erreur. |
| [FirstId](../../aspose.pdf.ai/listdataresponse-1/firstid/) { get; set; } | Obtient ou définit le premier ID dans la liste. |
| [HasMore](../../aspose.pdf.ai/listdataresponse-1/hasmore/) { get; set; } | Obtient ou définit une valeur indiquant s'il y a plus d'éléments dans la liste. |
| [HttpResponseHeaders](../../aspose.pdf.ai/baseresponse/httpresponseheaders/) { get; set; } | Obtient ou définit les en-têtes de réponse HTTP. |
| [HttpStatusCode](../../aspose.pdf.ai/baseresponse/httpstatuscode/) { get; set; } | Obtient ou définit le code d'état HTTP. |
| [IsSuccessful](../../aspose.pdf.ai/baseresponse/issuccessful/) { get; } | Indique si la réponse a été réussie. |
| [LastId](../../aspose.pdf.ai/listdataresponse-1/lastid/) { get; set; } | Obtient ou définit le dernier ID dans la liste. |
| [ReasonPhrase](../../aspose.pdf.ai/baseresponse/reasonphrase/) { get; } | Obtient la phrase de raison de l'erreur. |

### Voir aussi

* classe [DataResponse&lt;T&gt;](../dataresponse-1/)
* espace de noms [Aspose.Pdf.AI](../../aspose.pdf.ai/)
* assembly [Aspose.PDF](../../)