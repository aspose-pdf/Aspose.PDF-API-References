...
description: "Propriété RunCreateRequest. Obtient ou définit quel outil, le cas échéant, est appelé par le modèle. none signifie que le modèle n'appellera aucun outil et générera plutôt un message. auto est la valeur par défaut et indique que le modèle peut choisir entre générer un message ou appeler un ou plusieurs outils. required signifie que le modèle doit appeler un ou plusieurs outils avant de répondre à l'utilisateur. Spécifier un outil particulier comme type file_search ou type function avec le nom de fonction my_function oblige le modèle à appeler cet outil."
type: docs
weight: 130
url: /fr/net/aspose.pdf.ai/runcreaterequest/toolchoice/
---
