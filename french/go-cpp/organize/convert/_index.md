...
description: "Convertir un document PDF en un document PDF avec le format PDF spécifié."
type: docs
url: /fr/go-cpp/organize/convert/
---

_Convertir un document PDF en un document PDF avec le format PDF spécifié._
...
```go
// Énumération des actions d'erreurs de conversion possibles.
...
func main() {
	...
	// Convert(pdfFormat PdfFormat, action ConvertErrorAction) convertit le document PDF en PDF/A
	ok, logStr, err := pdf.Convert(asposepdf.PDF_A_2A, asposepdf.Delete)
	if err != nil {
		log.Fatal("Convert PDF/A error:", err)
	}

	// Imprimer le résultat de la conversion et le journal complet
	fmt.Printf("Convert PDF/A result: %v\n", ok)
	fmt.Printf("Convert PDF/A log:\n%s\n", logStr)

	// Enregistrer le document PDF converti
