...
type: docs
weight: 710
url: /fr/python-net/aspose.pdf.annotations/richmediaannotation/
---
...
Le type RichMediaAnnotation expose les membres suivants :
## Constructeurs
| Nom | Description |
| :- | :- |
| RichMediaAnnotation(page, rect) | Initialise une nouvelle instance de la classe RichMediaAnnotation |
## Propriétés
| Nom | Description |
| :- | :- |
...
| custom_player | Définit ou obtient le lecteur flash personnalisé pour lire les données vidéo/audio. |
| custom_flash_variables | Définit ou obtient les variables flash qui sont passées au lecteur. |
| contenu | Données du contenu Rich Media. |
| type | Obtient ou définit le type de contenu. Valeurs possibles : Audio, Vidéo. |
| activate_on | Événement qui active l'application. |
## Méthodes
| Nom | Description |
| :- | :- |
...
| accept(visitor) | Accepte le visiteur pour cette annotation. |
...
| add_custom_data(name, data) | Ajoute des données nommées personnalisées (par exemple requises pour le script flash). |
| set_content(file_name, audio) | Définit le flux de contenu. |
| set_poster(image_stream) | Définit l'affiche de l'annotation. |
