...
type: docs
weight: 680
url: /fr/python-net/aspose.pdf.annotations/redactionannotation/
---
...
Le type RedactionAnnotation expose les membres suivants :
## Constructeurs
| Nom | Description |
| :- | :- |
| RedactionAnnotation(document) | Initialise une nouvelle instance de la classe RedactionAnnotation |
| RedactionAnnotation(page, rect) | Initialise une nouvelle instance de la classe RedactionAnnotation |
## Propriétés
| Nom | Description |
| :- | :- |
...
| quad_point | Un tableau de nombres 8xN spécifiant les coordonnées de la région de contenu qui doit être supprimée. |
| default_appearance | Obtient ou définit la chaîne d'apparence par défaut à utiliser pour le formatage du texte. |
| fill_color | Obtient ou définit la couleur de remplissage de l'annotation. |
| border_color | Obtient ou définit la couleur de la bordure qui est dessinée lorsque la rédaction n'est pas active. |
| overlay_text | Texte à imprimer sur l'annotation de rédaction. |
| repeat | Si vrai, le texte superposé sera répété sur l'annotation. |
| text_alignment | Obtient ou définit l'alignement du texte superposé. |
## Méthodes
| Nom | Description |
| :- | :- |
| clone() | Aucun |
...
| flatten() | Aplati l'annotation, c.-à-d. supprime l'annotation et ajoute son |
| change_after_resize(transform) | Met à jour les paramètres et l'apparence, selon la transformation matricielle. |
| redact() | Aplati l'annotation et rédige le contenu de la page (c.-à-d. supprime le texte et l'image sous l'annotation de rédaction) |
