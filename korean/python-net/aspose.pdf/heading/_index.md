...
type: docs
weight: 460
url: /ko/python-net/aspose.pdf/heading/
---
...
Heading 유형은 다음 멤버를 노출합니다:
## 생성자
| 이름 | 설명 |
| :- | :- |
| Heading(level) | Heading 클래스의 새 인스턴스를 초기화합니다. |
## 속성
| 이름 | 설명 |
| :- | :- |
...
| toc_page | 이 제목을 포함하는 페이지를 가져옵니다. |
| top | 이 제목들의 상단 Y 좌표를 가져옵니다. |
| start_number | 제목 시작 번호를 가져옵니다. |
| is_auto_sequence | 제목이 자동으로 번호 매겨지는지 여부를 가져옵니다. |
| is_in_list | 제목이 목차 목록에 포함되는지 여부를 가져옵니다. |
| destination_page | 대상 페이지를 가져옵니다. |
| level | 레벨을 가져옵니다. |
| style | 스타일을 가져오거나 설정합니다. |
| user_label | 사용자 레이블을 가져오거나 설정합니다. |
## 메서드
| 이름 | 설명 |
| :- | :- |
| clone() | 제목을 복제합니다. |
...
| clone_with_segments() | 모든 세그먼트를 포함하여 제목을 복제합니다. |
