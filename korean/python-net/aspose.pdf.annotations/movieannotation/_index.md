...
description: "컴퓨터 화면과 스피커를 통해 표시되는 애니메이션 그래픽 및 사운드를 포함하는 영화 주석을 나타냅니다. 주석이 활성화되면 영화가 재생됩니다."
type: docs
weight: 480
url: /ko/python-net/aspose.pdf.annotations/movieannotation/
---
...
MovieAnnotation 유형은 다음 멤버를 노출합니다:
## 생성자
| 이름 | 설명 |
| :- | :- |
| MovieAnnotation(document, movie_file) | MovieAnnotation 클래스의 새 인스턴스를 초기화합니다. |
| MovieAnnotation(page, rect, movie_file) | MovieAnnotation 클래스의 새 인스턴스를 초기화합니다. |
## 속성
| 이름 | 설명 |
| :- | :- |
...
| 제목 | 영화 주석의 제목을 가져오거나 설정합니다. |
| file | 자체 설명이 가능한 영화 파일을 식별하는 파일 사양을 가져오거나 설정합니다. |
| 포스터 | 포스터 이미지를 영화에 표시할지 여부와 방식을 지정하는 플래그 또는 스트림을 가져오거나 설정합니다. true인 경우 포스터 이미지는 영화 파일에서 가져오며, false인 경우 포스터가 표시되지 않습니다. |
| 종횡비 | 영화의 경계 상자의 너비와 높이를 픽셀 단위로 가져오거나 설정합니다. |
| rotate | 영화를 페이지에 대해 시계 방향으로 회전시킬 각도를 가져오거나 설정합니다. 값은 90의 배수여야 합니다. |
## 메서드
| 이름 | 설명 |
| :- | :- |
