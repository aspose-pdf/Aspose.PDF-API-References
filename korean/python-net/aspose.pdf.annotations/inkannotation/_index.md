...
description: "하나 이상의 분리된 경로로 구성된 자유형 \"낙서\"를 나타냅니다."
type: docs
weight: 350
url: /ko/python-net/aspose.pdf.annotations/inkannotation/
---
...
InkAnnotation 유형은 다음 멤버를 노출합니다:
## 생성자
| 이름 | 설명 |
| :- | :- |
| InkAnnotation(document, ink_list) | InkAnnotation 클래스의 새 인스턴스를 초기화합니다. |
| InkAnnotation(page, rect, ink_list) | InkAnnotation 클래스의 새 인스턴스를 초기화합니다. |
| InkAnnotation(page, rect, ink_list) | InkAnnotation 클래스의 새 인스턴스를 초기화합니다. |
## 속성
| 이름 | 설명 |
| :- | :- |
...
| cap_style | 잉크 주석 선 끝의 스타일. |
| ink_list | 독립적인 선으로 표시되는 제스처 목록을 가져오거나 설정합니다. 이 선은 Point[] 배열로 표현됩니다. |
## 메서드
| 이름 | 설명 |
| :- | :- |
| clone() | 없음 |
...
| change_after_resize(transform) | 매트릭스 변환에 따라 InkList의 포인트를 업데이트합니다. |
