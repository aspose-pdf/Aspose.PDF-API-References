...
description: "PDF 문서에 비디오/오디오 데이터를 삽입할 수 있는 RichMediaAnnotation을 설명하는 클래스입니다."
type: docs
weight: 710
url: /ko/python-net/aspose.pdf.annotations/richmediaannotation/
---

## RichMediaAnnotation class

PDF 문서에 비디오/오디오 데이터를 삽입할 수 있는 RichMediaAnnotation을 설명하는 클래스입니다.

RichMediaAnnotation 유형은 다음 멤버를 노출합니다:
## 생성자
| 이름 | 설명 |
| :- | :- |
| RichMediaAnnotation(page, rect) | RichMediaAnnotation 클래스의 새 인스턴스를 초기화합니다. |
## 속성
| 이름 | 설명 |
| :- | :- |
...
| custom_player | 비디오/오디오 데이터를 재생하기 위한 사용자 지정 플래시 플레이어를 설정하거나 가져옵니다. |
| custom_flash_variables | 플레이어에 전달되는 플래시 변수를 설정하거나 가져옵니다. |
| content | Rich Media 콘텐츠의 데이터. |
| type | 콘텐츠 유형을 가져오거나 설정합니다. 가능한 값: Audio, Video. |
| activate_on | 응용 프로그램을 활성화하는 이벤트. |
## 메서드
| 이름 | 설명 |
| :- | :- |
| clone() | 이 인스턴스를 복제합니다.<br/>            가상 메서드. 항상 null을 반환합니다. |
| get_rectangle(consider_rotation) | 페이지 회전을 고려하여 주석의 사각형을 반환합니다. |
| accept(visitor) | 이 주석에 대한 방문자를 허용합니다. |
| flatten() | 주석 내용을 페이지에 직접 배치합니다,<br/>            주석 객체가 제거됩니다. |
| change_after_resize(transform) | 행렬 변환에 따라 매개변수와 외관을 업데이트합니다. |
| add_custom_data(name, data) | 맞춤 이름 데이터를 추가합니다 (예: 플래시 스크립트에 필요함). |
| set_content(file_name, audio) | 콘텐츠 스트림을 설정합니다. |
| set_poster(image_stream) | 주석의 포스터를 설정합니다. |
| update() | 지정된 매개변수로 데이터를 업데이트합니다. |
