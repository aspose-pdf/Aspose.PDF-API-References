...
description: "선 주석을 나타내는 클래스입니다."
type: docs
weight: 380
url: /ko/python-net/aspose.pdf.annotations/lineannotation/
---
...
LineAnnotation 유형은 다음 멤버를 제공합니다:
## 생성자
| 이름 | 설명 |
| :- | :- |
| LineAnnotation(document, start, end) | LineAnnotation 클래스의 새 인스턴스를 초기화합니다. |
| LineAnnotation(page, rect, start, end) | LineAnnotation 클래스의 새 인스턴스를 초기화합니다. |
## 속성
| 이름 | 설명 |
| :- | :- |
...
| starting | 선의 시작점을 가져오거나 설정합니다. |
| starting_style | 선 시작점에 대한 선 끝 스타일을 가져오거나 설정합니다. |
| ending | 선 끝점을 가져오거나 설정합니다. |
| ending_style | 선 끝점에 대한 끝 스타일을 가져오거나 설정합니다. |
| interior_color | 주석의 내부 색상을 가져오거나 설정합니다. |
| leader_line | 리더 라인의 길이를 가져오거나 설정합니다. |
| leader_line_extension | 리더 라인 확장의 길이를 가져오거나 설정합니다. |
| show_caption | 내용을 캡션으로 표시해야 하는지 여부를 결정하는 부울 플래그를 가져오거나 설정합니다. |
| leader_line_offset | 리더 라인 오프셋을 가져오거나 설정합니다. |
| caption_offset | 캡션 텍스트가 정상 위치에서 벗어난 오프셋을 가져오거나 설정합니다. |
| caption_position | 주석 캡션 위치를 가져오거나 설정합니다. |
| measure | 이 주석에 지정된 측정 단위입니다. |
| intent | 선 주석의 의도를 가져오거나 설정합니다. |
## 메서드
| 이름 | 설명 |
| :- | :- |
| clone() | 없음 |
...
| change_after_resize(transform) | 행렬 변환에 따라 시작점과 끝점을 업데이트합니다. |
