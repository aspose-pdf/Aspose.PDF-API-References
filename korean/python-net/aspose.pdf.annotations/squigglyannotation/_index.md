...
description: "문서 텍스트에 들쭉날쭉한 밑줄로 표시되는 squiggly 주석을 나타냅니다."
type: docs
weight: 780
url: /ko/python-net/aspose.pdf.annotations/squigglyannotation/
---

## SquigglyAnnotation class

문서 텍스트에 들쭉날쭉한 밑줄로 표시되는 squiggly 주석을 나타냅니다.

SquigglyAnnotation 유형은 다음 멤버를 제공합니다:
## 생성자
| 이름 | 설명 |
| :- | :- |
| SquigglyAnnotation(page, rect) | SquigglyAnnotation 클래스의 새 인스턴스를 초기화합니다. |
## 속성
| 이름 | 설명 |
| :- | :- |
...
| quad_points | n개의 사각형 좌표를 지정하는 점 배열을 가져오거나 설정합니다. 각 사각형은 주석이 적용된 텍스트에서 단어 또는 연속된 단어 그룹을 포함합니다. |
## 메서드
| 이름 | 설명 |
| :- | :- |
| clone() | 없음 |
...
| flatten() | 없음 |
| change_after_resize(transform) | QuadPoints를 매트릭스 변환에 따라 업데이트합니다. |
| get_marked_text() | 마크업 주석 아래의 텍스트를 문자열로 가져옵니다. |
| get_marked_text_fragments() | 마크업 주석 아래의 텍스트를 [TextFragmentCollection](/pdf/python-net/aspose.pdf.text/textfragmentcollection/) 로 가져옵니다. |
