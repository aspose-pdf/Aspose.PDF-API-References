...
description: "페이지에 직접 텍스트를 표시하는 자유 텍스트 주석을 나타냅니다. 일반 텍스트 주석과 달리 자유 텍스트 주석은 열림 또는 닫힘 상태가 없으며, 팝업 창에 표시되는 대신 텍스트가 항상 보입니다."
type: docs
weight: 260
url: /ko/python-net/aspose.pdf.annotations/freetextannotation/
---
...
FreeTextAnnotation 유형은 다음 멤버를 노출합니다:
## 생성자
| 이름 | 설명 |
| :- | :- |
| FreeTextAnnotation(document, appearance) | FreeTextAnnotation 클래스의 새 인스턴스를 초기화합니다 |
| FreeTextAnnotation(page, rect, appearance) | FreeTextAnnotation 클래스의 새 인스턴스를 초기화합니다 |
## 속성
| 이름 | 설명 |
| :- | :- |
...
| opacity | 주석을 그릴 때 사용할 상수 불투명도 값을 가져오거나 설정합니다. |
...
| starting_style | 라인 끝점의 라인 종료 스타일을 가져오거나 설정합니다.<br/>            이 속성은 더 이상 사용되지 않으며, EndingStyle를 사용하십시오. |
| ending_style | 라인 끝점의 라인 종료 스타일을 가져오거나 설정합니다. |
| justification | 주석 텍스트를 표시하는 데 사용되는 정렬(퀘이딩) 형태를 지정하는 코드를 가져오거나 설정합니다. |
| default_appearance | 텍스트 서식 지정에 사용되는 기본 외관 문자열을 가져오거나 설정합니다. |
| default_appearance_object | FreeText 주석의 기본 외관을 나타내는 객체. |
| intent | FreeText 주석의 의도를 가져오거나 설정합니다. |
| default_style | 기본 스타일 문자열을 가져오거나 설정합니다. |
| text_style | 외관에서 텍스트 스타일을 가져오거나 설정합니다. 텍스트 스타일이 변경되면 텍스트 외관이 업데이트됩니다. |
| rotate | 주석 회전 각도. |
| callout | 콜아웃 라인을 지정하는 점 배열. |
| text_rectangle | 두 사각형 간의 수치적 차이를 설명하는 사각형: 주석의 Rect 항목<br/>             및 해당 사각형 내부에 포함된 사각형. 내부 사각형은 주석�s 텍스트가 표시되어야 하는 영역입니다. |
## 메서드
| 이름 | 설명 |
| :- | :- |
| clone() | 없음 |
