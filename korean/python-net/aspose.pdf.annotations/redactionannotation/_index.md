...
description: "Redact 주석을 나타냅니다."
type: docs
weight: 680
url: /ko/python-net/aspose.pdf.annotations/redactionannotation/
---
...
RedactionAnnotation 유형은 다음 멤버를 제공합니다:
## 생성자
| 이름 | 설명 |
| :- | :- |
| RedactionAnnotation(document) | RedactionAnnotation 클래스의 새 인스턴스를 초기화합니다. |
| RedactionAnnotation(page, rect) | RedactionAnnotation 클래스의 새 인스턴스를 초기화합니다. |
## 속성
| 이름 | 설명 |
| :- | :- |
...
| quad_point | 제거될 콘텐츠 영역의 좌표를 지정하는 8xN 숫자 배열입니다. |
| default_appearance | 텍스트 서식 지정에 사용되는 기본 외관 문자열을 가져오거나 설정합니다. |
| fill_color | 주석을 채우는 색상을 가져오거나 설정합니다. |
| border_color | Redaction이 비활성화된 경우 그려지는 테두리 색상을 가져오거나 설정합니다. |
| overlay_text | Redact 주석에 인쇄할 텍스트입니다. |
| repeat | true인 경우 오버레이 텍스트가 주석에 반복됩니다. |
| text_alignment | 가져오거나 설정합니다. 오버레이 텍스트의 정렬입니다. |
## 메서드
| 이름 | 설명 |
| :- | :- |
| clone() | 없음 |
...
| flatten() | 주석을 평탄화합니다(즉, 주석을 제거하고 해당 내용을 추가합니다). |
| change_after_resize(transform) | 행렬 변환에 따라 매개변수와 외관을 업데이트합니다. |
| redact() | 주석을 평탄화하고 페이지 내용을 redact합니다(즉, redact된 주석 아래의 텍스트와 이미지를 제거합니다). |
