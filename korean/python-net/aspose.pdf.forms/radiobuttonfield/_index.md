...
description: "라디오 버튼 필드를 나타내는 클래스입니다."
type: docs
weight: 220
url: /ko/python-net/aspose.pdf.forms/radiobuttonfield/
---

## RadioButtonField class

라디오 버튼 필드를 나타내는 클래스입니다.

RadioButtonField 유형은 다음 멤버들을 노출합니다:
## 생성자
| 이름 | 설명 |
| :- | :- |
| RadioButtonField(page) | RadioButtonField 클래스의 새 인스턴스를 초기화합니다. |
| RadioButtonField(doc) | RadioButtonField 클래스의 새 인스턴스를 초기화합니다. |
## 속성
| 이름 | 설명 |
| :- | :- |
...
| full_name | 없음 |
| 모양 | 없음 |
| page_index | 이 RadioButton 필드를 포함하는 페이지의 인덱스를 가져옵니다. |
| on_activated | 없음 |
| highlighting | 없음 |
| parent | 없음 |
...
| 선택됨 | 선택된 항목의 인덱스를 가져오거나 설정합니다. 항목 번호는 1부터 시작합니다. |
| selected_items | 선택된 항목들의 배열을 가져오거나 설정합니다. 다중 선택 목록의 경우 배열에 둘 이상의 항목이 포함됩니다. 단일 선택 목록의 경우 하나의 항목만 포함됩니다. |
| options | 라디오 버튼의 옵션 컬렉션을 가져옵니다. |
| style | 필드 상자의 스타일. |
## Indexer
| 이름 | 설명 |
| :- | :- |
...
| 이름 | 설명 |
| :- | :- |
| add_option(option_name, rect) | 지정된 사각형을 사용하여 라디오 버튼 옵션을 추가합니다. |
| add_option(option_name) | 지정된 사각형을 사용하여 라디오 버튼 옵션을 추가합니다. |
| add_option(export, name) | 지정된 사각형을 사용하여 라디오 버튼 옵션을 추가합니다. |
| clone() | 없음 |
...
| set_position(point) | 라디오 버튼의 모든 하위 항목을 페이지의 지정된 위치로 이동합니다. |
| delete_option(option_name) | 옵션을 이름으로 삭제합니다. |
| add(new_item) | RadioButton 필드에 새 옵션 필드를 추가합니다. |
