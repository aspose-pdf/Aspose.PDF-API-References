...
description: "클래스는 바코드 필드를 나타냅니다."
type: docs
weight: 10
url: /ko/python-net/aspose.pdf.forms/barcodefield/
---
...
BarcodeField 유형은 다음 멤버를 노출합니다:
## 생성자
| 이름 | 설명 |
| :- | :- |
| BarcodeField(page, rect) | BarcodeField 클래스의 새 인스턴스를 초기화합니다 |
| BarcodeField(doc, rect) | BarcodeField 클래스의 새 인스턴스를 초기화합니다 |
## 속성
| 이름 | 설명 |
| :- | :- |
...
| multiline | 필드의 multiline 플래그를 가져오거나 설정합니다. Multiline이 true이면 필드에 여러 줄의 텍스트를 포함할 수 있습니다. |
| spell_check | 필드의 spellcheck 플래그를 가져오거나 설정합니다. true이면 필드가 맞춤법 검사를 수행합니다. |
| scrollable | 필드의 scrollable 플래그를 가져오거나 설정합니다. true이면 필드를 스크롤할 수 있습니다. |
| force_combs | 필드가 간격이 있는 위치로 나뉘는지를 나타내는 플래그를 가져오거나 설정합니다. |
| max_len | 필드 내 텍스트의 최대 길이를 가져오거나 설정합니다. |
| text_vertical_alignment | 주석에 대한 텍스트 수직 정렬을 가져오거나 설정합니다. |
| resolution | 바코드 객체가 렌더링되는 해상도(dpi)를 가져옵니다. |
| caption | 바코드 객체의 캡션을 가져옵니다. |
| symbology | 이 주석에 사용할 바코드 또는 글리프 기술을 지정합니다,<br/>            자세한 내용은 [symbology](/pdf/python-net/aspose.pdf.forms/barcodefield/)를 참조하십시오. |
| x_sym_width | 두 바코드 모듈 사이의 수평 거리(픽셀 단위)를 가져옵니다. |
| x_sym_height | 두 바코드 모듈 사이의 수직 거리(픽셀 단위)를 가져옵니다. <br/>            XSymHeight/XSymWidth 비율은 정수값이어야 합니다. <br/>            PDF417의 경우 허용 비율 범위는 1에서 4까지입니다. QRCode와 DataMatrix의 경우, <br/>            이 비율은 항상 1이어야 합니다. |
| ecc | 오류 정정 계수를 나타내는 정수 값을 가져옵니다. <br/>            PDF417의 경우 0에서 8 사이여야 합니다. QRCode의 경우 0에서 3 사이여야 합니다 <br/>            (0은 �L�, 1은 �M�, 2는 �Q�, 3은 �H�를 의미합니다). |
## Indexer
| 이름 | 설명 |
| :- | :- |
...
| add_image(image) | 이미지를 필드 리소스에 추가하고 그립니다. |
| add_barcode(code) | 필드에 바코드 128을 추가합니다. <br/>            필드 값이 코드에 적용되고 필드는 읽기 전용이 됩니다. |
