...
description: "서명 양식 필드를 나타냅니다."
type: docs
weight: 270
url: /ko/python-net/aspose.pdf.forms/signaturefield/
---
...
SignatureField 유형은 다음 멤버를 노출합니다:
## 생성자
| 이름 | 설명 |
| :- | :- |
| SignatureField(page, rect) | SignatureField 클래스의 새 인스턴스를 초기화합니다. |
| SignatureField(doc, rect) | SignatureField 클래스의 새 인스턴스를 초기화합니다. |
## 속성
| 이름 | 설명 |
| :- | :- |
...
| signature | 서명 객체를 가져옵니다.<br/>            이 객체는 공개키 암호 표준에 관한 서명 데이터를 포함합니다.<br/>            클래스 [PKCS1](/pdf/python-net/aspose.pdf.forms/pkcs1/), [PKCS7](/pdf/python-net/aspose.pdf.forms/pkcs7/) 및 [PKCS7Detached](/pdf/python-net/aspose.pdf.forms/pkcs7detached/) <br/>            은(는) 지원되는 모든 서명 객체 유형을 나타냅니다. |
## Indexer
| 이름 | 설명 |
| :- | :- |
...
| 이름 | 설명 |
| :- | :- |
| sign(signature, pfx, pass) | 이 서명 필드를 사용하여 문서에 서명합니다. |
| sign(signature) | 이 서명 필드를 사용하여 문서에 서명합니다. |
| extract_image() | 서명의 이미지를 JPEG 인코딩 스트림으로 추출합니다. |
| extract_image(format) | 서명의 이미지를 인코딩된 스트림으로 추출합니다. |
| clone() | 없음 |
...
| extract_certificate() | 단일 X.509 인증서를 DER 형식의 스트림으로 추출합니다. |
