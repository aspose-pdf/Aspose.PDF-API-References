---
title: "Append"
second_title: "C++를 통한 Go용 Aspose.PDF"
description: "다른 PDF 문서에서 페이지를 추가합니다."
type: docs
url: /ko/go-cpp/core/append/
---

_다른 PDF-document에서 페이지를 추가합니다._

```go
func (document *Document) Append(anotherdocument *Document) error
```

**Parameters**: 
  * **anotherdocument** - reference to PDF-document instance

**Return**: 
  * **error** - contains an error or nil if absent


**Example**:
```go
package main

import "github.com/aspose-pdf/aspose-pdf-go-cpp"
import "log"

func main() {

	// Open(filename string) filename을 사용하여 PDF-document을 엽니다
	pdf, err := asposepdf.Open("sample.pdf")
	if err != nil {
		log.Fatal(err)
	}

	// Close() PDF-document에 할당된 리소스를 해제합니다
	defer pdf.Close()

	// Open(filename string)는 파일 이름으로 다른 PDF-document를 엽니다
	anotherpdf, err := asposepdf.Open("sample1page.pdf")
	if err != nil {
		log.Fatal(err)
	}

	// Close() PDF-document에 할당된 리소스를 해제합니다
	defer anotherpdf.Close()

	// Append(anotherdocument *Document)는 다른 PDF-document에서 페이지를 추가합니다.
	err = pdf.Append(anotherpdf)
	if err != nil {
		log.Fatal(err)
	}

	// SaveAs(filename string)는 이전에 연 PDF-document를 새 파일 이름으로 저장합니다
	err = pdf.SaveAs("sample_Append.pdf")
	if err != nil {
		log.Fatal(err)
	}
}
```
