...
description: "Aspose.Pdf.Devices.GifDevice 클래스. PDF 문서 페이지를 GIF로 저장하는 데 도움이 되는 이미지 장치를 나타냅니다"
type: docs
weight: 3720
url: /ko/net/aspose.pdf.devices/gifdevice/
---
## GifDevice class

pdf 문서 페이지를 gif 형식으로 저장하는 데 도움이 되는 이미지 장치를 나타냅니다.
...
| 이름 | 설명 |
| --- | --- |
| [GifDevice](gifdevice/#constructor)() | `GifDevice` 클래스의 새 인스턴스를 기본 해상도로 초기화합니다. |
| [GifDevice](gifdevice/#constructor_2)(PageSize) | 제공된 페이지 크기와 기본 해상도(=150)로 `GifDevice` 클래스의 새 인스턴스를 초기화합니다. |
| [GifDevice](gifdevice/#constructor_1)(Resolution) | `GifDevice` 클래스의 새 인스턴스를 초기화합니다. 결과 이미지 파일의 해상도는 [`Resolution`](../resolution/) 클래스를 참조하십시오. |
| [GifDevice](gifdevice/#constructor_4)(int, int) | 제공된 이미지 차원과 기본 해상도(=150)로 `GifDevice` 클래스의 새 인스턴스를 초기화합니다. |
| [GifDevice](gifdevice/#constructor_3)(PageSize, Resolution) | 제공된 페이지 크기와 해상도로 `GifDevice` 클래스의 새 인스턴스를 초기화합니다. |
| [GifDevice](gifdevice/#constructor_5)(int, int, Resolution) | 제공된 이미지 차원과 해상도로 `GifDevice` 클래스의 새 인스턴스를 초기화합니다. |
...
| override [Process](../../aspose.pdf.devices/gifdevice/process/#process)(Page, Stream) | 페이지를 GIF로 변환하고 출력 스트림에 저장합니다. |
| [Process](../../aspose.pdf.devices/pagedevice/process/)(Page, string) | 지정된 page에 대해 일부 작업을 수행하고 결과를 파일에 저장합니다. |

## 예제

다음 예제는 PDF 파일을 GIF 이미지로 변환하는 방법을 보여줍니다.
