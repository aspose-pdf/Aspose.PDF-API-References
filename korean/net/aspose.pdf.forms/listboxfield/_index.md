...
description: Aspose.Pdf.Forms.ListBoxField 클래스. 클래스는 ListBox 필드를 나타냅니다.
type: docs
weight: 5130
...
| [IsSharedField](../../aspose.pdf.forms/field/issharedfield/) { get; set; } | Generator 지원을 위한 속성입니다. 필드가 머리글이나 바닥글에 추가될 때 사용됩니다. true인 경우 이 필드는 한 번만 생성되며 모든 페이지에서 그 외관이 표시됩니다. false인 경우 문서의 각 페이지에 대해 별도의 필드가 생성됩니다. |
...
| override [SelectedItems](../../aspose.pdf.forms/listboxfield/selecteditems/) { set; } | 다중 선택 목록에서 선택된 항목의 배열을 가져오거나 설정합니다. 단일 선택 목록의 경우 단일 항목이 있는 배열을 반환합니다. |
...
| [TopIndex](../../aspose.pdf.forms/listboxfield/topindex/) { get; set; } | 목록의 가장 위에 보이는 요소의 인덱스를 가져오거나 설정합니다. |
