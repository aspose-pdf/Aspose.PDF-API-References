...
description: Aspose.Pdf.Forms.Field 클래스. 아크로폼 필드의 기본 클래스
type: docs
weight: 5050
...
| 이름 | 설명 |
| --- | --- |
| [Field](field/)(Document) | 생성기에서 사용할 필드를 생성합니다. |
...
| [IsSharedField](../../aspose.pdf.forms/field/issharedfield/) { get; set; } | 생성기 지원을 위한 속성입니다. 필드가 머리글이나 바닥글에 추가될 때 사용됩니다. true이면 이 필드는 한 번만 생성되며 그 외관은 문서의 모든 페이지에서 표시됩니다. false이면 문서의 각 페이지에 대해 별도의 필드가 생성됩니다. |
...
| static [FitIntoRectangle](../../aspose.pdf.forms/field/fitintorectangle/) { get; set; } | true이면 글꼴 크기가 지정된 사각형에 맞도록 줄어듭니다. |
| static [MaxFontSize](../../aspose.pdf.forms/field/maxfontsize/) { get; set; } | 필드 내용에 사용할 수 있는 최대 글꼴 크기입니다. -1은 크기를 확인하지 않습니다. |
| static [MinFontSize](../../aspose.pdf.forms/field/minfontsize/) { get; set; } | 필드 내용에 사용할 수 있는 최소 글꼴 크기입니다. -1은 크기를 확인하지 않습니다. |
...
| [CopyTo](../../aspose.pdf.forms/field/copyto/#copyto)(WidgetAnnotation[], int) | 지정된 인덱스에서 시작하여 이 필드의 하위 필드를 배열에 복사합니다. |
...
| [ImportValueFromJson](../../aspose.pdf.forms/field/importvaluefromjson/#importvaluefromjson)(Stream) | 필드의 전체 이름이 정확히 일치하는 경우 JSON 스트림에서 지정된 필드로 데이터를 가져옵니다. |
