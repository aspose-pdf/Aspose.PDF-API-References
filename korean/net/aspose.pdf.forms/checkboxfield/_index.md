---
title: "클래스 CheckboxField"
second_title: "Aspose.PDF 용 .NET API 참조"
description: "Aspose.Pdf.Forms.CheckboxField 클래스. 체크박스 필드를 나타내는 클래스"
type: docs
weight: 5100
url: /ko/net/aspose.pdf.forms/checkboxfield/
---
## CheckboxField class
...
| 이름 | 설명 |
| --- | --- |
| [CheckboxField](checkboxfield/#constructor_1)(Document) | Generator와 함께 사용할 생성자. |
| [CheckboxField](checkboxfield/#constructor_2)(Document, Rectangle) | CheckboxField 클래스용 생성자. |
| [CheckboxField](checkboxfield/#constructor_3)(Page, Rectangle) | CheckboxField 클래스용 생성자. |
...
| override [ActiveState](../../aspose.pdf.forms/checkboxfield/activestate/) { get; set; } | 현재 주석 외관 상태를 가져오거나 설정합니다. |
| [AllowedStates](../../aspose.pdf.forms/checkboxfield/allowedstates/) { get; } | 허용된 상태 목록을 반환합니다. |
| [AlternateName](../../aspose.pdf.forms/field/alternatename/) { get; set; } | 필드의 대체 이름을 가져오거나 설정합니다 (실제 필드 이름 대신 사용자 인터페이스에서 필드를 식별할 때 사용되는 대체 필드 이름). 대체 이름은 Adobe Acrobat에서 필드 툴팁으로 사용됩니다. |
...
| [Checked](../../aspose.pdf.forms/checkboxfield/checked/) { get; set; } | 체크 박스의 상태를 가져오거나 설정합니다. |
...
| [ExportValue](../../aspose.pdf.forms/checkboxfield/exportvalue/) { get; set; } | CheckBox 필드의 내보내기 값을 가져오거나 설정합니다. |
...
| [IsGroup](../../aspose.pdf.forms/field/isgroup/) { get; } | 가져오거나 설정합니다. 이 필드가 비종단 필드(즉, 필드 그룹)인지 여부를 나타내는 boolean 값입니다. |
...
| [IsSharedField](../../aspose.pdf.forms/field/issharedfield/) { get; set; } | Generator 지원을 위한 속성입니다. 필드가 헤더 또는 푸터에 추가될 때 사용됩니다. true인 경우, 이 필드는 한 번만 생성되며 그 외관이 모든 Document 페이지에 표시됩니다. false인 경우, 각 Document 페이지마다 별도의 필드가 생성됩니다. |
| [IsSynchronized](../../aspose.pdf.forms/field/issynchronized/) { get; } | 사전이 동기화된 경우 true를 반환합니다. |
| [Item](../../aspose.pdf.forms/field/item/) { get; } | 이 필드에 포함된 서브필드를 서브필드 이름으로 가져옵니다. (2개의 인덱서) |
| [MappingName](../../aspose.pdf.forms/field/mappingname/) { get; set; } | 필드의 매핑 이름을 가져오거나 설정합니다. 이 이름은 Document에서 대화형 양식 필드 데이터를 내보낼 때 사용됩니다. |
...
| override [PageIndex](../../aspose.pdf.forms/field/pageindex/) { get; } | 이 필드를 포함하는 Page의 인덱스를 가져옵니다. |
...
| override [Rect](../../aspose.pdf.forms/field/rect/) { get; set; } | 필드 Rectangle을 가져오거나 설정합니다. |
...
| [Style](../../aspose.pdf.forms/checkboxfield/style/) { get; set; } | 체크 박스의 스타일을 가져오거나 설정합니다. |
| [SyncRoot](../../aspose.pdf.forms/field/syncroot/) { get; } | 동기화 객체. |
...
| override [Value](../../aspose.pdf.forms/checkboxfield/value/) { get; set; } | 체크 박스 필드의 값을 가져오거나 설정합니다. |
...
| [AddOption](../../aspose.pdf.forms/checkboxfield/addoption/#addoption)(string) | 체크 박스 그룹에 새 체크 박스를 추가합니다. 이 그룹에서는 동시에 하나의 체크 박스만 선택될 수 있습니다. 새 체크 박스는 그룹의 하단에 추가됩니다. |
| [AddOption](../../aspose.pdf.forms/checkboxfield/addoption/#addoption_1)(string, Rectangle) | 체크 박스 그룹에 새 체크 박스를 추가합니다. 이 그룹에서는 동시에 하나의 체크 박스만 선택될 수 있습니다. |
| [AddOption](../../aspose.pdf.forms/checkboxfield/addoption/#addoption_2)(string, int, Rectangle) | 체크 박스 그룹에 새 체크 박스를 추가합니다. 이 그룹에서는 동시에 하나의 체크 박스만 선택될 수 있습니다. |
| virtual [ChangeAfterResize](../../aspose.pdf.annotations/annotation/changeafterresize/)(Matrix) | 행렬 변환에 따라 매개변수와 모양을 업데이트합니다. |
| override [Clone](../../aspose.pdf.forms/checkboxfield/clone/)() | 체크 박스를 복제합니다. |
| [CopyTo](../../aspose.pdf.forms/field/copyto/)(WidgetAnnotation[], int) | 이 필드의 하위 필드를 지정된 인덱스부터 배열에 복사합니다. |
| [ExecuteFieldJavaScript](../../aspose.pdf.forms/field/executefieldjavascript/)(JavascriptAction) | 필드에 대해 지정된 JavaScript 동작을 실행합니다. |
...
| [ExportValueToJson](../../aspose.pdf.forms/field/exportvaluetojson/)(Stream, bool) | 지정된 필드의 내용을 JSON 스트림으로 내보냅니다. 버튼 필드 값은 내보내지 않습니다. |
| override [Flatten](../../aspose.pdf.forms/field/flatten/)() | 이 필드를 제거하고 값을 페이지에 직접 배치합니다. |
...
| [ImportValueFromJson](../../aspose.pdf.forms/field/importvaluefromjson/)(Stream) | 필드 전체 이름이 정확히 일치하는 경우 JSON 스트림에서 지정된 필드로 데이터를 가져옵니다. |
...
| [Recalculate](../../aspose.pdf.forms/field/recalculate/)() | 양식의 모든 계산된 필드를 다시 계산합니다. |
| virtual [SetPosition](../../aspose.pdf.forms/field/setposition/)(Point) | 필드의 위치를 설정합니다. |

## 예제

이 예제는 다중값 체크 박스 필드를 만드는 방법을 보여줍니다.
...
// 첫 번째 체크 박스 그룹 옵션 값을 설정합니다.
checkbox.ExportValue = "option 1";

// 기존 옵션 바로 아래에 새 옵션을 추가합니다.
checkbox.AddOption("option 2");

// 지정된 사각형에 새 옵션을 추가합니다.
