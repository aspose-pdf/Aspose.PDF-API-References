...
---
## RichTextBoxField 클래스

클래스는 리치 텍스트 편집기 구성 요소를 설명합니다.
...
| 이름 | 설명 |
| --- | --- |
| [RichTextBoxField](richtextboxfield/)(Page, Rectangle) | 리치 텍스트 박스 필드의 생성자. |
...
| [IsGroup](../../aspose.pdf.forms/field/isgroup/) { get; } | 이 필드가 비종단 필드(즉, 필드 그룹)인지 여부를 나타내는 부울 값을 가져오거나 설정합니다. |
...
| [IsSharedField](../../aspose.pdf.forms/field/issharedfield/) { get; set; } | 생성기 지원을 위한 속성입니다. 필드가 머리글 또는 바닥글에 추가될 때 사용됩니다. true인 경우 이 필드는 한 번 생성되며 모든 페이지에서 그 외관이 표시됩니다. false인 경우 문서의 각 페이지에 대해 별도의 필드가 생성됩니다. |
