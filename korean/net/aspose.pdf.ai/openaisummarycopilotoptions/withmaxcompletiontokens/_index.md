---
title: OpenAISummaryCopilotOptions.WithMaxCompletionTokens
second_title: Aspose.PDF for .NET API Reference
description: OpenAISummaryCopilotOptions 메서드. 요약 코파일럿 옵션의 최대 완료 토큰을 설정합니다.
type: docs
weight: 90
url: /ko/net/aspose.pdf.ai/openaisummarycopilotoptions/withmaxcompletiontokens/
---
## OpenAISummaryCopilotOptions.WithMaxCompletionTokens 메서드

요약 코파일럿 옵션의 최대 완료 토큰을 설정합니다.

```csharp
public OpenAISummaryCopilotOptions WithMaxCompletionTokens(int? maxCompletionTokens)
```

| 매개변수 | 유형 | 설명 |
| --- | --- | --- |
| maxCompletionTokens | Nullable`1 | 설정할 최대 완료 토큰입니다. |

### 반환 값

[`OpenAISummaryCopilotOptions`](../)의 현재 인스턴스입니다.

### 참조

* 클래스 [OpenAISummaryCopilotOptions](../)
* 네임스페이스 [Aspose.Pdf.AI](../../../aspose.pdf.ai/)
* 어셈블리 [Aspose.PDF](../../../)