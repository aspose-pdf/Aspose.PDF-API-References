...
| 이름 | 설명 |
| --- | --- |
| [ExcelSaveOptions](excelsaveoptions/)() | 기본 생성자입니다. |
...
| [InsertBlankColumnAtFirst](../../aspose.pdf/excelsaveoptions/insertblankcolumnatfirst/) { get; set; } | 워크시트의 첫 번째 열로 빈 열을 삽입해야 하는 경우 true로 설정합니다. 기본값은 false입니다. 즉, 빈 열이 삽입되지 않습니다. |
| [MinimizeTheNumberOfWorksheets](../../aspose.pdf/excelsaveoptions/minimizethenumberofworksheets/) { get; set; } | 결과 워크북에서 워크시트 수를 최소화해야 하는 경우 true로 설정합니다. 기본값은 false입니다. 즉, 각 PDF 페이지를 별도의 워크시트로 저장합니다. |
| [SaveFormat](../../aspose.pdf/saveoptions/saveformat/) { get; } | 데이터 저장 형식입니다. |
| [UniformWorksheets](../../aspose.pdf/excelsaveoptions/uniformworksheets/) { get; set; } | 문서 전반에 걸쳐 균일한 열 분할을 사용하려면 true로 설정합니다. 기본값은 false입니다. 즉, 각 페이지에 대해 열 분할이 독립적입니다. |
...
| [TryMergeAdjacentSameBackgroundImages](../../aspose.pdf/unifiedsaveoptions/trymergeadjacentsamebackgroundimages/) | 때때로 PDF에는 서로 가까이 배치된 여러 동일한 타일 배경 이미지로 구성된 배경 이미지(페이지 또는 테이블 셀)가 포함됩니다. 이러한 경우 대상 형식의 렌더러(예: DOCS 형식의 MsWord)는 때때로 배경 이미지의 부분 사이에 가시적인 경계를 생성합니다. 이는 이미지 가장자리 부드럽게 처리하는 기술(안티 앨리어싱)이 Acrobat Reader와 다르기 때문입니다. 내보낸 문서에 동일한 배경 이미지의 부분 사이에 가시적인 경계가 포함된 것처럼 보이는 경우 이 설정을 사용하여 원하지 않는 효과를 제거해 보십시오. 주의! 이 품질 최적화는 일반적으로 변환 속도를 크게 저하시킵니다. 따라서 정말 필요한 경우에만 이 옵션을 사용하십시오. |
