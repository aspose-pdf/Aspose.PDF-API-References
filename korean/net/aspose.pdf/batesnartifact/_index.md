...
description: Aspose.Pdf.BatesNArtifact 클래스. 클래스는 Bates 번호 매기기 아티팩트를 설명합니다.
type: docs
weight: 2850
url: /ko/net/aspose.pdf/batesnartifact/
---
## BatesNArtifact 클래스

클래스는 Bates 번호 매기기 아티팩트를 설명합니다.
...
| 이름 | 설명 |
| --- | --- |
| [BatesNArtifact](batesnartifact/)() | `BatesNArtifact` 클래스의 새 인스턴스를 초기화합니다. 이 생성자는 내부적이며 기본값으로 헤더 아티팩트 인스턴스를 생성합니다. |

## 속성

| 이름 | 설명 |
| --- | --- |
| [ArtifactHorizontalAlignment](../../aspose.pdf/artifact/artifacthorizontalalignment/) { get; set; } | 아티팩트의 수평 정렬입니다. 위치가 명시적으로 지정된 경우(위치 속성에서) 이 값은 무시됩니다. |
| [ArtifactVerticalAlignment](../../aspose.pdf/artifact/artifactverticalalignment/) { get; set; } | 아티팩트의 수직 정렬입니다. 위치가 명시적으로 지정된 경우(위치 속성에서) 이 값은 무시됩니다. |
| [BottomMargin](../../aspose.pdf/artifact/bottommargin/) { get; set; } | 아티팩트의 하단 여백입니다. 위치가 명시적으로 지정된 경우(위치 속성에서) 이 값은 무시됩니다. |
...
| [LeftMargin](../../aspose.pdf/artifact/leftmargin/) { get; set; } | 아티팩트의 왼쪽 여백입니다. 위치가 명시적으로 지정된 경우(위치 속성에서) 이 값은 무시됩니다. |
| [Lines](../../aspose.pdf/artifact/lines/) { get; } | 다중 행 텍스트 아티팩트의 행입니다. |
| [NumberOfDigits](../../aspose.pdf/batesnartifact/numberofdigits/) { get; set; } | Bates 번호 매기기를 위한 숫자의 개수를 가져오거나 설정합니다. 값은 3에서 15 사이여야 합니다. 3 미만의 값이 설정되면 3으로 조정됩니다. 15 초과의 값이 설정되면 15로 조정됩니다. 기본값은 6입니다. |
...
| [Prefix](../../aspose.pdf/batesnartifact/prefix/) { get; set; } | Bates 번호에 추가할 접두사를 가져오거나 설정합니다. |
| [Rectangle](../../aspose.pdf/artifact/rectangle/) { get; } | 아티팩트의 사각형을 가져옵니다. |
| [RightMargin](../../aspose.pdf/artifact/rightmargin/) { get; set; } | 아티팩트의 오른쪽 여백입니다. 위치가 명시적으로 지정된 경우(위치 속성에서) 이 값은 무시됩니다. |
| [Rotation](../../aspose.pdf/artifact/rotation/) { get; set; } | 아티팩트 회전 각도를 가져오거나 설정합니다. |
| [StartNumber](../../aspose.pdf/batesnartifact/startnumber/) { get; set; } | Bates 번호 매기기를 위한 시작 번호를 가져오거나 설정합니다. 값은 1 이상이어야 합니다. 1 미만의 값이 설정되면 1로 조정됩니다. |
...
| [Suffix](../../aspose.pdf/batesnartifact/suffix/) { get; set; } | Bates 번호에 추가할 접미사를 가져오거나 설정합니다. |
...
| [TopMargin](../../aspose.pdf/artifact/topmargin/) { get; set; } | 아티팩트의 상단 여백입니다. 위치가 명시적으로 지정된 경우(위치 속성에서) 이 값은 무시됩니다. |
...
| 이름 | 설명 |
| --- | --- |
| [BeginUpdates](../../aspose.pdf/artifact/beginupdates/)() | 지연된 업데이트를 시작합니다. 성능을 개선하기 위해 동일한 아티팩트에 여러 변경을 해야 하는 경우 이 기능을 사용하십시오. 일반적으로 아티팩트 속성이 변경될 때마다 아티팩트 연산자가 변경됩니다. 이로 인해 아티팩트가 변경될 때마다 페이지 내용이 변경됩니다. 이 효과를 피하려면 모든 아티팩트 업데이트를 StartUpdates/SaveUpdates 호출 사이에 넣으십시오. 이렇게 하면 페이지 내용을 한 번만 변경할 수 있습니다. |
