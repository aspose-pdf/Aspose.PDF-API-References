---
title: "클래스 PageNumberStamp"
second_title: "Aspose.PDF 용 .NET API 참조"
description: "Aspose.Pdf.PageNumberStamp 클래스. 페이지 번호 스탬프를 나타내며 페이지 번호 매기에 사용됩니다."
type: docs
weight: 8370
...
페이지 번호 스탬프를 나타내며 페이지 번호 매기에 사용됩니다.
...
| 이름 | 설명 |
| --- | --- |
| [PageNumberStamp](pagenumberstamp/#constructor)() | `PageNumberStamp` 클래스의 새 인스턴스를 초기화합니다. 형식은 "#"(으)로 설정됩니다. |
| [PageNumberStamp](pagenumberstamp/#constructor_1)(FormattedText) | 서식이 지정된 텍스트로 PageNumberStamp를 생성합니다. |
...
| [AutoAdjustFontSizeToFitStampRectangle](../../aspose.pdf/textstamp/autoadjustfontsizetofitstamprectangle/) { get; set; } | 활성화된 경우, 글꼴 크기가 자동으로 조정되어 스탬프 사각형 크기인 [`Width`](../textstamp/width/) 및 [`Height`](../textstamp/height/)에 맞춥니다. 기본 너비와 높이는 page 사각형에서 파생됩니다. |
...
| [Draw](../../aspose.pdf/textstamp/draw/) { get; set; } | 이 속성은 페이지에 스탬프가 그려지는 방식을 결정합니다. Draw = true이면 스탬프가 그래픽 연산자로 그려지고, draw = false이면 텍스트로 그려집니다. |
| [FontSize](../../aspose.pdf/textstamp/fontsize/) { get; } | 스탬프가 배치된 후 실제 글꼴 크기입니다. ('AutoAdjustFontSizeToFitStampRectangle' 옵션이 활성화된 경우, 생성자를 통해 제공된 초기 글꼴 크기와 다를 수 있습니다.) |
| [Format](../../aspose.pdf/pagenumberstamp/format/) { get; set; } | page 번호를 스탬프하기 위한 문자열 값입니다. 값에는 '#' 문자가 포함되어야 하며, 스탬프 과정에서 해당 문자로 page 번호가 교체됩니다. |
| override [Height](../../aspose.pdf/textstamp/height/) { get; set; } | page 상의 스탬프 가로 정렬을 가져오거나 설정합니다. |
| [HorizontalAlignment](../../aspose.pdf/stamp/horizontalalignment/) { get; set; } | 스탬프의 왼쪽 여백을 가져오거나 설정합니다. |
| [Justify](../../aspose.pdf/textstamp/justify/) { get; set; } | 텍스트 정렬을 정의합니다. 이 속성이 true로 설정되면 텍스트의 왼쪽 및 오른쪽 가장자리가 정렬됩니다. 기본값: false. |
| [LeftMargin](../../aspose.pdf/stamp/leftmargin/) { get; set; } | 스탬프 불투명도를 나타내는 값을 가져오거나 설정합니다. 값은 0.0에서 1.0 사이이며, 기본값은 1.0입니다. |
| [MaxRowWidth](../../aspose.pdf/textstamp/maxrowwidth/) { get; set; } | WordWrap 옵션에 대한 최대 행 높이입니다. |
| [NoCharacterBehavior](../../aspose.pdf/textstamp/nocharacterbehavior/) { get; set; } | 글꼴에 요청된 문자가 없을 경우 동작을 정의하는 모드를 가져오거나 설정합니다. |
...
| [ReplacementFont](../../aspose.pdf/textstamp/replacementfont/) { get; set; } | 사용자 글꼴에 필요한 문자가 없을 경우 대체에 사용되는 글꼴을 가져오거나 설정합니다. |
...
| [Scale](../../aspose.pdf/textstamp/scale/) { get; set; } | 텍스트의 스케일링을 정의합니다. 이 속성이 true이고 Width 값이 지정된 경우, 텍스트가 지정된 너비에 맞게 스케일됩니다. |
| [StartingNumber](../../aspose.pdf/pagenumberstamp/startingnumber/) { get; set; } | 시작 page 번호의 값을 가져오거나 설정합니다. 다른 pages는 이 값부터 번호가 매겨집니다. |
| [TextAlignment](../../aspose.pdf/textstamp/textalignment/) { get; set; } | 스탬프 내부 텍스트의 정렬입니다. |
...
| [TreatYIndentAsBaseLine](../../aspose.pdf/textstamp/treatyindentasbaseline/) { get; set; } | 텍스트 배치를 위한 좌표 원점을 정의합니다. TreatYIndentAsBaseLine = true인 경우 (Draw = true일 때 기본값) YIndent 값은 텍스트 기준선으로 처리됩니다. TreatYIndentAsBaseLine = false인 경우 (Draw = false일 때 기본값) YIndent 값은 텍스트의 하단(하강선)으로 처리됩니다. |
| [Value](../../aspose.pdf/textstamp/value/) { get; set; } | 페이지에 스탬프로 사용되는 문자열 값을 가져오거나 설정합니다. |
...
| [WordWrapMode](../../aspose.pdf/textstamp/wordwrapmode/) { get; set; } | 텍스트 렌더링을 위한 워드 랩 모드를 가져오거나 설정합니다. |
...
| override [Put](../../aspose.pdf/pagenumberstamp/put/)(Page) | page 번호를 추가합니다. |
