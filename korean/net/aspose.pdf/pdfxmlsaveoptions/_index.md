...
| 이름 | 설명 |
| --- | --- |
| [PdfXmlSaveOptions](pdfxmlsaveoptions/)() | 기본 생성자입니다. |
...
| [WarningHandler](../../aspose.pdf/saveoptions/warninghandler/) { get; set; } | 생성된 경고를 처리하는 콜백입니다. WarningHandler는 Continue 또는 Abort를 지정하는 ReturnAction 열거형 항목을 반환합니다. Continue는 기본 동작이며 저장 작업이 계속되지만, 사용자가 Abort를 반환할 경우 저장 작업이 중단되어야 합니다. |
...
| [TryMergeAdjacentSameBackgroundImages](../../aspose.pdf/unifiedsaveoptions/trymergeadjacentsamebackgroundimages/) | 때때로 PDF에는 서로 가까이 배치된 여러 동일한 타일 배경 이미지로 구성된 페이지 또는 테이블 셀의 배경 이미지가 포함됩니다. 이러한 경우 대상 형식의 렌더러(예: DOCS 형식의 MsWord)는 때때로 배경 이미지의 부분 사이에 가시적인 경계를 생성하며, 이는 이미지 가장자리 부드럽게 처리하는 기술(안티 앨리어싱)이 Acrobat Reader와 다르기 때문입니다. 내보낸 문서에 동일한 배경 이미지의 부분 사이에 가시적인 경계가 포함된 것처럼 보인다면, 이 설정을 사용하여 원하지 않는 효과를 제거해 보십시오. 주의! 이 품질 최적화는 일반적으로 변환 속도를 크게 저하시킵니다. 따라서 이 옵션은 정말 필요한 경우에만 사용하십시오. |
