---
title: "ColorBarAnnotation 클래스"
second_title: "Aspose.PDF 용 .NET API 참조"
description: "Aspose.Pdf.Annotations.ColorBarAnnotation 클래스. ColorBarAnnotation 주석을 나타내는 클래스. Property Color는 무시되고 대신 ColorsOfCMYK 색상이 사용됩니다. 생성 시 너비와 높이 비율에 따라 주석의 방향이 가로 또는 세로로 결정됩니다. 다음으로 주석 사각형이 TrimBox 밖에 있는지 확인하고, 그렇지 않으면 주석의 방향을 고려하여 가장 가까운 TrimBox 외부 위치로 이동됩니다. 주석이 TrimBox 밖에 맞도록 너비와 높이를 줄일 수 있습니다. 레이아웃을 위한 공간이 없을 경우 너비/높이를 0으로 설정할 수 있으며, 이 경우 주석은 페이지에 존재하지만 표시되지 않습니다."
type: docs
weight: 1690
...
| 이름 | 설명 |
| --- | --- |
| [ColorBarAnnotation](colorbarannotation/)(Page, Rectangle, ColorsOfCMYK) | 지정된 페이지에 새로운 ColorBar 주석을 생성합니다. |
...
| override [AnnotationType](../../aspose.pdf.annotations/colorbarannotation/annotationtype/) { get; } | 주석 유형을 가져옵니다. |
...
| [ColorOfCMYK](../../aspose.pdf.annotations/colorbarannotation/colorofcmyk/) { get; set; } | 주석이 그려지는 색상(시안, 마젠타, 옐로, 블랙 중 하나)을 가져오거나 설정합니다. |
...
| 이름 | 설명 |
| --- | --- |
| override [Accept](../../aspose.pdf.annotations/colorbarannotation/accept/)(AnnotationSelector) | Annotation을 처리하기 위해 방문자 객체를 받아들입니다. |
| override [ChangeAfterResize](../../aspose.pdf.annotations/colorbarannotation/changeafterresize/)(Matrix) | 필요한 경우 매트릭스 변환 및 TrimBox 외부 이동에 따라 매개변수와 외관을 업데이트합니다. |
