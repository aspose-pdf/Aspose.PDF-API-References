...
description: "Converter um PDF-documento em um PDF-documento com o formato PDF especificado."
type: docs
url: /pt/go-cpp/organize/convert/
---

_Converta um PDF-document em um PDF-document com o formato PDF especificado._
...
```go
// Enumeração da ação de erros de conversão possíveis.
...
func main() {
	...
	// Convert(pdfFormat PdfFormat, action ConvertErrorAction) converte PDF-document para PDF/A
	ok, logStr, err := pdf.Convert(asposepdf.PDF_A_2A, asposepdf.Delete)
	if err != nil {
		log.Fatal("Convert PDF/A error:", err)
	}

	// Imprima o resultado da conversão e o log completo
	fmt.Printf("Convert PDF/A result: %v\n", ok)
	fmt.Printf("Convert PDF/A log:\n%s\n", logStr)

	// Salve o PDF-document convertido
