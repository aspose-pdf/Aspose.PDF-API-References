---
title: "PolylineAnnotation"
second_title: "Aspose.PDF para Python via .NET Referência da API"
description: "Representa anotação de polilinha que é semelhante a um polígono, exceto que o primeiro e o último vértice não são conectados implicitamente."
type: docs
weight: 660
url: /pt/python-net/aspose.pdf.annotations/polylineannotation/
---

## PolylineAnnotation class

Representa anotação de polilinha que é semelhante a um polígono, exceto que o primeiro e o último vértice não são conectados implicitamente.

O tipo PolylineAnnotation expõe os seguintes membros:
## Construtores
| Nome | Descrição |
| :- | :- |
| PolylineAnnotation(page, rect, vertices) | Inicializa uma nova instância da classe PolylineAnnotation |
## Propriedades
| Nome | Descrição |
| :- | :- |
| vertical_alignment | Nenhum |
| horizontal_alignment | Nenhum |
| margem | Nenhum |
| is_first_paragraph_in_column | Nenhum |
| is_kept_with_next | Nenhum |
| is_in_new_page | Nenhum |
| is_in_line_paragraph | Nenhum |
| hiperlink | Nenhum |
| z_index | Nenhum |
| atualizar_aparência_ao_converter | Nenhum |
| usar_subconjunto_de_fonte | Nenhum |
| sinalizadores | Nenhum |
| tipo_de_anotação | Obtém o tipo da anotação. |
| largura | Nenhum |
| ações | Nenhum |
| altura | Nenhum |
| retângulo | Nenhum |
| conteúdo | Nenhum |
| nome | Nenhum |
| modificado | Nenhum |
| cor | Nenhum |
| borda | Nenhum |
| estado_ativo | Nenhum |
| características | Nenhum |
| estados | Nenhum |
| alinhamento | Nenhum |
| alinhamento_horizontal_do_texto | Nenhum |
| nome_completo | Nenhum |
| aparência | Nenhum |
| índice_da_página | Nenhum |
| title | Obtém ou define um texto que será exibido na barra de título da anotação. |
| rich_text | Obtém ou define uma string de texto rico a ser exibida na janela pop-up quando a anotação for aberta. |
| creation_date | Obtém a data e hora em que a anotação foi criada. |
| subject | Obtém o texto que representa a descrição do objeto. |
| popup | Anotação pop-up para inserir ou editar o texto associado a esta anotação. |
| opacidade | Obtém ou define o valor constante de opacidade a ser usado ao renderizar a anotação. |
| in_reply_to | Uma referência à anotação que esta anotação está "em resposta a".<br/>            Ambas as anotações devem estar na mesma página do documento. |
| reply_type | Uma string que especifica o relacionamento (o "tipo de resposta") entre esta anotação<br/>            e a especificada por InReplyTo. |
| measure | Unidades de medida especificadas para esta anotação. |
| vertices | Obtém ou define um array de pontos que representa as coordenadas horizontais e verticais de cada vértice. |
| interior_color | Obtém ou define a cor interna com a qual preencher as terminações de linha da anotação. |
| starting_style | Obtém ou define o estilo da primeira terminação de linha. |
| ending_style | Obtém ou define o estilo da segunda terminação de linha. |
| intent | Obtém ou define a intenção da anotação de polígono ou polilinha. |
## Métodos
| Nome | Descrição |
| :- | :- |
| clone() | Nenhum |
| get_rectangle(consider_rotation) | Nenhum |
| accept(visitor) | Aceita um objeto visitante para processar a anotação. |
| flatten() | Nenhum |
| change_after_resize(transform) | Atualiza os pontos em Vertices, de acordo com a transformação matricial. |

### Veja Também

* namespace [aspose.pdf.annotations](/pdf/python-net/aspose.pdf.annotations/)
* assembly [Aspose.PDF](/pdf/python-net/)

