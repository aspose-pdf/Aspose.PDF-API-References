...
type: docs
weight: 680
url: /pt/python-net/aspose.pdf.annotations/redactionannotation/
---
...
O tipo RedactionAnnotation expõe os seguintes membros:
## Construtores
| Nome | Descrição |
| :- | :- |
| RedactionAnnotation(document) | Inicializa uma nova instância da classe RedactionAnnotation |
| RedactionAnnotation(page, rect) | Inicializa uma nova instância da classe RedactionAnnotation |
## Propriedades
| Nome | Descrição |
| :- | :- |
...
| quad_point | Um array de números 8xN especificando as coordenadas da região de conteúdo que se destina a ser removida. |
| default_appearance | Obtém ou define a string de aparência padrão a ser usada na formatação do texto. |
| fill_color | Obtém ou define a cor para preencher a anotação. |
| border_color | Obtém ou define a cor da borda que é desenhada quando a redação não está ativa. |
| overlay_text | Texto a ser impresso na anotação de redação. |
| repeat | Se verdadeiro, o texto sobreposto será repetido na anotação. |
| text_alignment | Obtém ou define. Alinhamento do Texto Sobreposto. |
## Métodos
| Nome | Descrição |
| :- | :- |
| clone() | Nenhum |
...
| flatten() | Achata a anotação, ou seja, remove a anotação e adiciona seu |
| change_after_resize(transform) | Atualiza parâmetros e aparência, de acordo com a transformação da matriz. |
| redact() | Achata a anotação e reda o conteúdo da página (ou seja, remove texto e imagem sob a anotação redigida) |
