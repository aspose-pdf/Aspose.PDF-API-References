...
description: Evento PdfViewer. Ocorre antes do início da impressão e permite fornecer manipuladores de impressão personalizados em vez do padrão
type: docs
weight: 200
url: /pt/net/aspose.pdf.facades/pdfviewer/customprint/
---
## Evento PdfViewer.CustomPrint

Ocorre antes do início da impressão e permite fornecer manipuladores de impressão personalizados em vez do padrão.
...
O exemplo demonstra como imprimir a partir do Aspose.PDF em sistemas Linux. O seguinte código é voltado principalmente para impressão a partir do Aspose.PDF em sistemas Linux. Usuários de sistemas Windows podem continuar usando a implementação de impressão padrão do PdfViewer sem fornecer um manipulador CustomPrint.

### Pré-requisitos

1. No sistema do servidor de impressão, o CUPS deve estar instalado e configurado:
* sudo apt update && apt install cups
* sudo service cups start
* se você vai imprimir documentos no mesmo sistema onde o aplicativo habilitado para Aspose.PDF está em execução, não precisará de configuração adicional do CUPS. Se precisar imprimir de um sistema diferente, consulte a documentação do CUPS sobre como permitir acesso ao servidor de impressão via rede.
2. Uma impressora pode ser configurada usando a interface web do CUPS. Opcionalmente, você pode usar uma impressora PDF virtual:
* sudo apt install printer-driver-cups-pdf
* sudo service cups restart
* por favor, certifique-se de que a impressora PDF virtual apareceu na lista de impressoras disponíveis na interface web do CUPS (em http://localhost:631/printers/ com as configurações padrão do CUPS)
3. Se o seu sistema cliente (onde o aplicativo habilitado para Aspose.PDF está em execução) for diferente do servidor de impressão, você também precisará instalar e executar o CUPS lá:
* sudo apt update && apt install cups
* sudo service cups start

### Como imprimir um documento usando o comando lp
...
### Como imprimir um documento usando o ipptool
