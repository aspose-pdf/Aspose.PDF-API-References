...
description: "コンピュータ画面とスピーカーで表示されるアニメーション画像と音声を含むムービー注釈を表します。注釈が有効になると、ムービーが再生されます。"
type: docs
weight: 480
url: /ja/python-net/aspose.pdf.annotations/movieannotation/
---

## MovieAnnotation class

コンピュータ画面とスピーカーで表示されるアニメーション画像と音声を含むムービー注釈を表します。注釈が有効になると、ムービーが再生されます。

MovieAnnotation 型は次のメンバーを公開します：
## コンストラクター
| 名前 | 説明 |
| :- | :- |
| MovieAnnotation(document, movie_file) | MovieAnnotation クラスの新しいインスタンスを初期化します |
| MovieAnnotation(page, rect, movie_file) | MovieAnnotation クラスの新しいインスタンスを初期化します |
## プロパティ
| 名前 | 説明 |
| :- | :- |
...
| タイトル | 映画注釈のタイトルを取得または設定します。 |
| file | 自己記述型映画ファイルを識別するファイル仕様を取得または設定します。 |
| poster | 映画を表すポスター画像を表示するかどうか、またその表示方法を指定するフラグまたはストリームを取得または設定します。true の場合、ポスター画像は映画ファイルから取得されます。false の場合、ポスターは表示されません。 |
| aspect | 映画のバウンディングボックスの幅と高さ（ピクセル単位）を取得または設定します。 |
| rotate | 映画がページに対して時計回りに回転する角度（度数）を取得または設定します。値は 90 の倍数でなければなりません。 |
## メソッド
| 名前 | 説明 |
| :- | :- |
