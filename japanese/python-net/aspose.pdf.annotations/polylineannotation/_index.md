...
description: "ポリゴンに似たポリライン注釈を表しますが、最初と最後の頂点は暗黙的に接続されません。"
type: docs
weight: 660
url: /ja/python-net/aspose.pdf.annotations/polylineannotation/
---

## PolylineAnnotation class

ポリゴンに似たポリライン注釈を表しますが、最初と最後の頂点は暗黙的に接続されません。

PolylineAnnotation 型は次のメンバーを公開します：
## コンストラクター
| 名前 | 説明 |
| :- | :- |
| PolylineAnnotation(page, rect, vertices) | PolylineAnnotation クラスの新しいインスタンスを初期化します |
## プロパティ
| 名前 | 説明 |
| :- | :- |
...
| vertices | 各頂点の水平および垂直座標を表すポイントの配列を取得または設定します。 |
| interior_color | 注釈の線端を塗りつぶすための内部色を取得または設定します。 |
| starting_style | 最初の線端のスタイルを取得または設定します。 |
| ending_style | 2番目の線端のスタイルを取得または設定します。 |
| intent | ポリゴンまたはポリライン注釈の意図を取得または設定します。 |
## メソッド
| 名前 | 説明 |
| :- | :- |
| clone() | None |
...
| flatten() | None |
| change_after_resize(transform) | 行列変換に従って Vertices のポイントを更新します。 |
