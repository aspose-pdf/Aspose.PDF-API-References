...
description: "クラスはファイル添付アノテーションを記述します。"
type: docs
weight: 170
url: /ja/python-net/aspose.pdf.annotations/fileattachmentannotation/
---

## FileAttachmentAnnotation class

クラスはファイル添付アノテーションを記述します。

FileAttachmentAnnotation 型は次のメンバーを公開します:
## コンストラクター
| 名前 | 説明 |
| :- | :- |
| FileAttachmentAnnotation(page, rect, file_spec) | FileAttachmentAnnotation クラスの新しいインスタンスを初期化します |
## プロパティ
| 名前 | 説明 |
| :- | :- |
...
| opacity | アイコンの不透明度を 0 から 1 の範囲で取得または設定します：0 - 完全に透明、1 - 完全に不透明。 |
...
| file | このアノテーションに関連付けられたファイルの仕様です。 |
| icon | アノテーションの表示に使用するアイコンを取得または設定します。 |
## メソッド
| 名前 | 説明 |
| :- | :- |
| clone() | None |
| get_rectangle(consider_rotation) | ページの回転を考慮した注釈の矩形を返します。 |
| accept(visitor) | アノテーションを処理するためのビジタオブジェクトを受け入れます。 |
