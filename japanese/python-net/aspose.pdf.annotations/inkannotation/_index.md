...
description: "1つ以上の非連続パスで構成されるフリーハンドの\\\"scribble\\\"を表します。"
type: docs
weight: 350
url: /ja/python-net/aspose.pdf.annotations/inkannotation/
---

## InkAnnotation class

1つ以上の非連続パスで構成されるフリーハンドの\"scribble\"を表します。

InkAnnotation 型は次のメンバーを公開します：
## コンストラクター
| 名前 | 説明 |
| :- | :- |
| InkAnnotation(document, ink_list) | InkAnnotation クラスの新しいインスタンスを初期化します |
| InkAnnotation(page, rect, ink_list) | InkAnnotation クラスの新しいインスタンスを初期化します |
| InkAnnotation(page, rect, ink_list) | InkAnnotation クラスの新しいインスタンスを初期化します |
## プロパティ
| 名前 | 説明 |
| :- | :- |
...
| cap_style | インク注釈の線端のスタイル。 |
| ink_list | 独立した線として表される Point[] 配列で表現されたジェスチャーのリストを取得または設定します。 |
## メソッド
| 名前 | 説明 |
| :- | :- |
| clone() | None |
...
| change_after_resize(transform) | マトリックス変換に従って InkList のポイントを更新します。 |
