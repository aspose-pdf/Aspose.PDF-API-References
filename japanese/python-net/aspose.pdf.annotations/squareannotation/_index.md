...
description: "四角形アノテーションを表すクラスです。"
type: docs
weight: 770
url: /ja/python-net/aspose.pdf.annotations/squareannotation/
---
...
SquareAnnotation 型は次のメンバーを公開します：
## コンストラクター
| 名前 | 説明 |
| :- | :- |
| SquareAnnotation(document) | SquareAnnotation クラスの新しいインスタンスを初期化します |
| SquareAnnotation(page, rect) | SquareAnnotation クラスの新しいインスタンスを初期化します |
## プロパティ
| 名前 | 説明 |
| :- | :- |
...
| interior_color | アノテーションの矩形または楕円を塗りつぶすための内部色。 |
| frame | 2つの矩形間の数値的差異を表す矩形：<br/>            アノテーションの Rect エントリと、基になる正方形または円の実際の境界。 |
## メソッド
| 名前 | 説明 |
| :- | :- |
| clone() | None |
| get_rectangle(consider_rotation) | None |
| accept(visitor) | アノテーションを処理するビジターを受け入れます。 |
| flatten() | None |
