...
description: "ラジオボタンフィールドを表すクラスです。"
type: docs
weight: 220
url: /ja/python-net/aspose.pdf.forms/radiobuttonfield/
---
...
RadioButtonField 型は以下のメンバーを公開します:
## コンストラクター
| 名前 | 説明 |
| :- | :- |
| RadioButtonField(page) | RadioButtonField クラスの新しいインスタンスを初期化します。 |
| RadioButtonField(doc) | RadioButtonField クラスの新しいインスタンスを初期化します。 |
## プロパティ
| 名前 | 説明 |
| :- | :- |
...
| full_name | None |
| appearance | None |
| page_index | この RadioButton フィールドを含むページのインデックスを取得します。 |
| on_activated | None |
| highlighting | None |
| parent | None |
...
| selected | 選択された項目のインデックスを取得または設定します。項目の番号は 1 から始まります。 |
| selected_items | 選択された項目の配列を取得または設定します。マルチセレクトリストの場合、配列には複数の項目が含まれます。シングルセレクトリストの場合、単一の項目が含まれます。 |
| options | ラジオボタンのオプションコレクションを取得します。 |
| style | フィールドボックスのスタイルです。 |
## Indexer
| 名前 | 説明 |
| :- | :- |
...
| 名前 | 説明 |
| :- | :- |
| add_option(option_name, rect) | 指定された矩形でラジオボタンのオプションを追加します。 |
| add_option(option_name) | 指定された矩形でラジオボタンのオプションを追加します。 |
| add_option(export, name) | 指定された矩形でラジオボタンのオプションを追加します。 |
| clone() | None |
...
| set_position(point) | ラジオボタンのすべてのサブアイテムをページ上の指定された位置に移動します。 |
| delete_option(option_name) | オプションをその名前で削除します。 |
| add(new_item) | RadioButton フィールドに新しいオプションフィールドを追加します。 |
