...
description: "クラスはバーコードフィールドを表します。"
type: docs
weight: 10
url: /ja/python-net/aspose.pdf.forms/barcodefield/
---
...
BarcodeField 型は次のメンバーを公開します：
## コンストラクター
| 名前 | 説明 |
| :- | :- |
| BarcodeField(page, rect) | BarcodeField クラスの新しいインスタンスを初期化します |
| BarcodeField(doc, rect) | BarcodeField クラスの新しいインスタンスを初期化します |
## プロパティ
| 名前 | 説明 |
| :- | :- |
...
| resolution | バーコードオブジェクトがレンダリングされる解像度（dpi）を取得します。 |
| caption | バーコードオブジェクトのキャプションを取得します。 |
| symbology | この注釈で使用するバーコードまたはグリフ技術を指定します、<br/>            詳細は [symbology](/pdf/python-net/aspose.pdf.forms/barcodefield/) を参照してください。 |
| x_sym_width | 2つのバーコードモジュール間の水平距離（ピクセル単位）を取得します。 |
| x_sym_height | 2つのバーコードモジュール間の垂直距離（ピクセル単位）を取得します。 <br/>            XSymHeight/XSymWidth の比率は整数でなければなりません。 <br/>            PDF417 の場合、許容される比率範囲は 1 から 4 です。QRCode と DataMatrix の場合、 <br/>            この比率は常に 1 である必要があります |
| ecc | 誤り訂正係数を表す整数値を取得します。 <br/>            PDF417 の場合、0 から 8 の範囲です。QRCode の場合、0 から 3 の範囲です <br/>            (0 は �L�、1 は �M�、2 は �Q�、3 は �H� を表します)。 |
## Indexer
| 名前 | 説明 |
| :- | :- |
