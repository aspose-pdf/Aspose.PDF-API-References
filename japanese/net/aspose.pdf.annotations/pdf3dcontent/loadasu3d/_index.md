---
title: "PDF3DContent.LoadAsU3D"
second_title: "Aspose.PDF for .NET API リファレンス"
description: "PDF3DContent メソッド。指定されたファイル名の 3D コンテンツを U3D 形式でロードします"
type: docs
weight: 70
url: /ja/net/aspose.pdf.annotations/pdf3dcontent/loadasu3d/
---
## LoadAsU3D(string) {#loadasu3d_2}

指定されたファイル名で3DコンテンツをU3D形式としてロードします。

```csharp
public void LoadAsU3D(string filename)
```

| パラメーター | タイプ | 説明 |
| --- | --- | --- |
| filename | String | ファイル名。 |

### 関連項目

* class [PDF3DContent](../)
* namespace [Aspose.Pdf.Annotations](../../../aspose.pdf.annotations/)
* assembly [Aspose.PDF](../../../)

---

## LoadAsU3D(Stream) {#loadasu3d_1}

ストリームから3DコンテンツをU3D形式としてロードします。

```csharp
public void LoadAsU3D(Stream stream)
```

| パラメーター | タイプ | 説明 |
| --- | --- | --- |
| stream | Stream | 3D コンテンツストリーム。 |

### 関連項目

* class [PDF3DContent](../)
* namespace [Aspose.Pdf.Annotations](../../../aspose.pdf.annotations/)
* assembly [Aspose.PDF](../../../)

---

## LoadAsU3D(byte[]) {#loadasu3d}

バイト配列から3DコンテンツをU3D形式としてロードします。

```csharp
public void LoadAsU3D(byte[] stream)
```

| パラメーター | タイプ | 説明 |
| --- | --- | --- |
| stream | Byte[] | ストリーム。 |

### 関連項目

* class [PDF3DContent](../)
* namespace [Aspose.Pdf.Annotations](../../../aspose.pdf.annotations/)
* assembly [Aspose.PDF](../../../)


