...
description: Aspose.Pdf.LogicalStructure.WarichuChildElement クラス。論理構造における Warichu の子要素の基本クラスを表します。
type: docs
weight: 6860
url: /ja/net/aspose.pdf.logicalstructure/warichuchildelement/
---
## WarichuChildElement クラス

論理構造における Warichu の子要素の基本クラスを表します。
...
| [RemoveAndMoveItsChildObjectsToItsParent](../../aspose.pdf.logicalstructure/structureelement/removeandmoveitschildobjectstoitsparent/)(bool) | 構造から要素を削除し、親オブジェクトからの参照、子オブジェクトからの参照、およびドキュメントからの対応するオブジェクトを削除します。削除されたオブジェクトの子オブジェクトをその以前の親の子オブジェクトコレクションに、削除されたオブジェクトのインデックスから開始して挿入します。 |
