...
description: "RunThreadCreateRequest プロパティ。モデルが出力すべき形式を取得または設定します。GPT4o、GPT4 Turbo、そして gpt3.5turbo1106 以降のすべての GPT3.5 Turbo モデルと互換性があります。type json_object に設定すると JSON モードが有効になり、モデルが生成するメッセージが有効な JSON であることが保証されます。JSON モードを使用する場合は、システムメッセージまたはユーザーメッセージでモデルに JSON を生成するよう指示する必要があります。指示がないと、モデルはトークン上限に達するまで空白文字を無限に生成し続け、長時間実行されているように見えるリクエストになることがあります。また、生成が max_tokens を超えた、または会話が最大コンテキスト長を超えたことを示す finish_reasonlength により、メッセージ内容が部分的に切り取られることがあります。"
type: docs
weight: 80
