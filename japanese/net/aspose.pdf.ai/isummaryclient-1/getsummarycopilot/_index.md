---
title: ISummaryClient1.GetSummaryCopilot
second_title: Aspose.PDF for .NET API Reference
description: ISummaryClient メソッド。指定されたオプションを持つ ISummaryCopilot のインスタンスを取得します
type: docs
weight: 10
url: /ja/net/aspose.pdf.ai/isummaryclient-1/getsummarycopilot/
---
## ISummaryClient&lt;TOptions&gt;.GetSummaryCopilot メソッド

指定されたオプションを持つ [`ISummaryCopilot`](../../isummarycopilot/) のインスタンスを取得します。

```csharp
public ISummaryCopilot GetSummaryCopilot(ISummaryCopilotOptions<TOptions> options)
```

| パラメーター | 型 | 説明 |
| --- | --- | --- |
| options | ISummaryCopilotOptions`1 | サマリーコパイロットのオプション。 |

### 戻り値

[`ISummaryCopilot`](../../isummarycopilot/) のインスタンス。

### 参照

* インターフェース [ISummaryCopilot](../../isummarycopilot/)
* インターフェース [ISummaryCopilotOptions&lt;TOptions&gt;](../../isummarycopilotoptions-1/)
* インターフェース [ISummaryClient&lt;TOptions&gt;](../)
* 名前空間 [Aspose.Pdf.AI](../../../aspose.pdf.ai/)
* アセンブリ [Aspose.PDF](../../../)