---
title: "JavascriptExtensionsException.JavascriptExtensionsException"
second_title: "Aspose.PDF for .NET API リファレンス"
description: "JavascriptExtensionsException コンストラクタ。JavascriptExtensionsException クラスの新しいインスタンスを初期化します。"
type: docs
weight: 10
url: /ja/net/aspose.pdf/javascriptextensionsexception/javascriptextensionsexception/
---
## JavascriptExtensionsException(string) {#constructor_1}

[`JavascriptExtensionsException`](../) クラスの新しいインスタンスを初期化します。

```csharp
public JavascriptExtensionsException(string message)
```

| パラメーター | タイプ | 説明 |
| --- | --- | --- |
| メッセージ | String | メッセージ。 |

### 関連項目

* class [JavascriptExtensionsException](../)
* namespace [Aspose.Pdf](../../../aspose.pdf/)
* assembly [Aspose.PDF](../../../)

---

## JavascriptExtensionsException(string, Exception) {#constructor_2}

指定されたエラーメッセージと、この例外の原因となる内部例外への参照を使用して、[`JavascriptExtensionsException`](../) クラスの新しいインスタンスを初期化します。

```csharp
public JavascriptExtensionsException(string message, Exception innerException)
```

| パラメーター | タイプ | 説明 |
| --- | --- | --- |
| メッセージ | String | 例外の原因を説明するエラーメッセージです。 |
| innerException | Exception | 現在の例外の原因となる例外、または内部例外が指定されていない場合は null 参照（Visual Basic では Nothing）です。 |

### 関連項目

* class [JavascriptExtensionsException](../)
* namespace [Aspose.Pdf](../../../aspose.pdf/)
* assembly [Aspose.PDF](../../../)

---

## JavascriptExtensionsException(Exception) {#constructor}

内部例外への参照（この例外の原因）を使用して、[`JavascriptExtensionsException`](../) クラスの新しいインスタンスを初期化します。

```csharp
public JavascriptExtensionsException(Exception innerException)
```

| パラメーター | タイプ | 説明 |
| --- | --- | --- |
| innerException | Exception | 現在の例外の原因となる例外、または内部例外が指定されていない場合は null 参照（Visual Basic では Nothing）です。 |

### 関連項目

* class [JavascriptExtensionsException](../)
* namespace [Aspose.Pdf](../../../aspose.pdf/)
* assembly [Aspose.PDF](../../../)


