...
TrySplitToEnd メソッドは SplitToEnd メソッドと同様ですが、操作が失敗した場合に TrySplitToEnd メソッドは例外をスローしません。
...
この操作の後、CloseConcatedStreams が指定されていない限り、ストリームは閉じられません。TrySplitToEnd メソッドは SplitToEnd メソッドと同様ですが、操作が失敗した場合に TrySplitToEnd メソッドは例外をスローしません。
