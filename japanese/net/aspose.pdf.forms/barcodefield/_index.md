...
description: Aspose.Pdf.Forms.BarcodeField クラス。クラスはバーコードフィールドを表します
type: docs
weight: 4950
...
| [Caption](../../aspose.pdf.forms/barcodefield/caption/) { get; } | バーコードオブジェクトのキャプションを取得します。 |
...
| [ECC](../../aspose.pdf.forms/barcodefield/ecc/) { get; } | エラー訂正係数を表す整数値を取得します。PDF417の場合は0から8まで、QRCodeの場合は0から3まで（0は「L」、1は「M」、2は「Q」、3は「H」）。 |
...
| [Resolution](../../aspose.pdf.forms/barcodefield/resolution/) { get; } | バーコードオブジェクトがレンダリングされる解像度（ドット毎インチ dpi）を取得します。 |
...
| [Symbology](../../aspose.pdf.forms/barcodefield/symbology/) { get; } | このアノテーションで使用されるバーコードまたはグリフ技術を指定します。詳細については [`Symbology`](./symbology/) を参照してください。 |
...
| [XSymHeight](../../aspose.pdf.forms/barcodefield/xsymheight/) { get; } | 2 つのバーコードモジュール間の垂直距離をピクセル単位で取得します。比率 XSymHeight/XSymWidth は整数値でなければなりません。PDF417 の場合、許容される比率範囲は 1 から 4 です。QRCode と DataMatrix の場合、この比率は常に 1 でなければなりません。 |
| [XSymWidth](../../aspose.pdf.forms/barcodefield/xsymwidth/) { get; } | 2 つのバーコードモジュール間の水平距離をピクセル単位で取得します。 |
