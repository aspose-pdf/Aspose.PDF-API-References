...
type: docs
weight: 1530
url: /ja/java/com.aspose.pdf/fitbhexplicitdestination/
---
**Inheritance:**
...
ページの垂直座標 top がウィンドウの上端に位置し、ページの内容がバウンディングボックスの幅全体がウィンドウ内に収まるように拡大された、明示的な表示先を表します。top が null の場合は、そのパラメータの現在値を変更せずに保持することを意味します。

## コンストラクタ

| コンストラクタ | 説明 |
| --- | --- |
| [FitBHExplicitDestination](#FitBHExplicitDestination-com.aspose.pdf.Document-int-double-) | リモートの明示的なデスティネーションを作成します。 |
| [FitBHExplicitDestination](#FitBHExplicitDestination-int-double-) | リモートの明示的なデスティネーションを作成します。 |
| [FitBHExplicitDestination](#FitBHExplicitDestination-com.aspose.pdf.Page-double-) | インスタンスを作成し、DOM ページオブジェクトと top パラメータで初期化します。 |
...
| [toString](#toString--) | オブジェクトの状態を文字列値に変換します。例: \"1 FitBH 100\"。 |
...
オブジェクトの状態を文字列値に変換します。例: \"1 FitBH 100\"。
