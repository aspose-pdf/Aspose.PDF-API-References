...
description: "Convertir un documento PDF en un documento PDF con el formato PDF especificado."
type: docs
url: /es/go-cpp/organize/convert/
---

_Convertir un documento PDF en un documento PDF con el formato PDF especificado._
...
```go
// Enumeración de la posible acción de errores de conversión.
...
func main() {
	...
	// Convert(pdfFormat PdfFormat, action ConvertErrorAction) convierte el documento PDF a PDF/A
	ok, logStr, err := pdf.Convert(asposepdf.PDF_A_2A, asposepdf.Delete)
	if err != nil {
		log.Fatal("Convert PDF/A error:", err)
	}

	// Imprimir el resultado de la conversión y el registro completo
	fmt.Printf("Convert PDF/A result: %v\n", ok)
	fmt.Printf("Convert PDF/A log:\n%s\n", logStr)

	// Guardar documento PDF convertido
