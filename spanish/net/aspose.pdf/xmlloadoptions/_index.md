---
title: XmlLoadOptions
second_title: Referencia de API de Aspose.PDF para .NET
description: Representa opciones para cargar/importar un archivo XML en un documento pdf.
type: docs
weight: 7440
url: /es/net/aspose.pdf/xmlloadoptions/
---
## XmlLoadOptions class

Representa opciones para cargar/importar un archivo XML en un documento pdf.

```csharp
public class XmlLoadOptions : LoadOptions
```

## Constructores

| Nombre | Descripción |
| --- | --- |
| [XmlLoadOptions](xmlloadoptions#constructor)() | Crea[`XmlLoadOptions`](../xmlloadoptions) objeto sin datos xsl. |
| [XmlLoadOptions](xmlloadoptions#constructor_1)(Stream) | Crea[`XmlLoadOptions`](../xmlloadoptions) objeto con datos xsl. |
| [XmlLoadOptions](xmlloadoptions#constructor_2)(string) | Crea[`XmlLoadOptions`](../xmlloadoptions) objeto con datos xsl. |

## Propiedades

| Nombre | Descripción |
| --- | --- |
| [LoadFormat](../../aspose.pdf/loadoptions/loadformat) { get; } | Representa el formato de archivo que[`LoadOptions`](../loadoptions) describe. |
| [WarningHandler](../../aspose.pdf/loadoptions/warninghandler) { get; set; } | Devolución de llamada para manejar cualquier advertencia generada. WarningHandler devuelve el elemento de enumeración ReturnAction especificando Continuar o Anular. Continuar es la acción predeterminada y la operación de carga continúa; sin embargo, el usuario también puede devolver Anular, en cuyo caso la operación de carga debe cesar. |
| [XslStream](../../aspose.pdf/xmlloadoptions/xslstream) { get; } | Obtiene datos xsl para convertir xml en documento pdf. |

### Ver también

* class [LoadOptions](../loadoptions)
* espacio de nombres [Aspose.Pdf](../../aspose.pdf)
* asamblea [Aspose.PDF](../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.PDF.dll -->
