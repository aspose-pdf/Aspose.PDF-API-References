---
title: EpubLoadOptions.PageSizeAdjustmentMode
second_title: Aspose.PDF for .NET API Reference
description: Campo EpubLoadOptions. ¡ATENCIÓN! La función está implementada pero aún no se ha puesto en la API pública debido a un problema bloqueador en la capa OSHARED revelado para el documento de muestra. Representa el modo de uso del tamaño de página durante la conversión. Formatos, generalmente tienen un diseño flotante, por lo que permite ajustar el tamaño de página requerido. Pero a veces el contenido tiene posiciones horizontales o tamaños especificados que no permiten colocar el contenido en el tamaño de página requerido. En tal caso, podemos definir qué se debe hacer en este caso. Es decir, cuando el tamaño del contenido no se ajusta al tamaño de página inicial requerido del documento PDF resultante.
type: docs
weight: 60
url: /es/net/aspose.pdf/epubloadoptions/pagesizeadjustmentmode/
---
## Campo EpubLoadOptions.PageSizeAdjustmentMode

¡ATENCIÓN! La función está implementada pero aún no se ha puesto en la API pública debido a un problema bloqueador en la capa OSHARED revelado para el documento de muestra. Representa el modo de uso del tamaño de página durante la conversión. Los formatos (como HTML, EPUB, etc.) generalmente tienen un diseño flotante, por lo que permite ajustar el tamaño de página requerido. Pero a veces el contenido tiene posiciones horizontales o tamaños especificados que no permiten colocar el contenido en el tamaño de página requerido. En tal caso, podemos definir qué se debe hacer en este caso (es decir, cuando el tamaño del contenido no se ajusta al tamaño de página inicial requerido del documento PDF resultante).

```csharp
public PageSizeAdjustmentModes PageSizeAdjustmentMode;
```

### Ver También

* enum [PageSizeAdjustmentModes](../../loadoptions.pagesizeadjustmentmodes/)
* class [EpubLoadOptions](../)
* namespace [Aspose.Pdf](../../../aspose.pdf/)
* assembly [Aspose.PDF](../../../)