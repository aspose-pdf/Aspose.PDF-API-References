...
Преобразовать документ в формат MHT. Этот код был экспериментальным, он использовался во время работ, связанных с https://pdf.aspose.com/jira/browse/PDFNEWNET-36340, не будет запущен в производство, потому что есть кросс-браузерные проблемы с созданным MHT, поэтому его можно использовать. в будущем если ваще то надо будет самому MHT создать. PDFNEWNET-36340 устранен с использованием URL-адресов DataSceme (встраивание данных в HTML http://en.wikipedia.org/wiki/Data\_URI\_scheme) Так вот, это преобразование действительно сейчас не используется.
