...
description: "Aspose.Pdf.Forms.Field класс. Базовый класс для полей acro-форм"
type: docs
weight: 5170
...
| Имя | Описание |
| --- | --- |
| [Field](field/)(Document) | Создаёт поле для использования в Generator. |
...
| static [FitIntoRectangle](../../aspose.pdf.forms/field/fitintorectangle/) { get; set; } | Если true, то размер шрифта будет уменьшен, чтобы поместить текст в указанный прямоугольник. |
| static [MaxFontSize](../../aspose.pdf.forms/field/maxfontsize/) { get; set; } | Максимальный размер шрифта, который может использоваться для содержимого поля. -1 чтобы не проверять размер. |
| static [MinFontSize](../../aspose.pdf.forms/field/minfontsize/) { get; set; } | Минимальный размер шрифта, который может использоваться для содержимого поля. -1 чтобы не проверять размер. |
...
| [CopyTo](../../aspose.pdf.forms/field/copyto/#copyto)(WidgetAnnotation[], int) | Копирует подполе(ы) этого поля в массив, начиная с указанного индекса. |
...
| [ImportValueFromJson](../../aspose.pdf.forms/field/importvaluefromjson/#importvaluefromjson)(Stream) | Импортирует данные в указанные поля из потока JSON, основываясь на точном совпадении полных имён полей. |
