...
description: Класс Aspose.Pdf.Forms.BarcodeField. Класс представляет поле штрих-кода
type: docs
weight: 4950
url: /ru/net/aspose.pdf.forms/barcodefield/
---
## Класс BarcodeField
...
| Имя | Описание |
| --- | --- |
| [BarcodeField](barcodefield/#constructor)(Document, Rectangle) | Инициализирует новый экземпляр класса `BarcodeField`. |
| [BarcodeField](barcodefield/#constructor_1)(Page, Rectangle) | Инициализирует новый экземпляр класса `BarcodeField`. |
...
| [AlternateName](../../aspose.pdf.forms/field/alternatename/) { get; set; } | Получает или устанавливает альтернативное имя поля (Альтернативное имя поля, которое будет использоваться вместо фактического имени поля, где бы поле ни идентифицировалось в пользовательском интерфейсе). Альтернативное имя используется в качестве подсказки поля в Adobe Acrobat. |
...
| [Caption](../../aspose.pdf.forms/barcodefield/caption/) { get; } | Получает заголовок объекта штрих-кода. |
...
| [ECC](../../aspose.pdf.forms/barcodefield/ecc/) { get; } | Получает целочисленное значение, представляющее коэффициент коррекции ошибок. Для PDF417 должен быть от 0 до 8. Для QRCode должен быть от 0 до 3 (0 для 'L', 1 для 'M', 2 для 'Q' и 3 для 'H'). |
...
| [ForceCombs](../../aspose.pdf.forms/textboxfield/forcecombs/) { get; set; } | Получает или устанавливает флаг, который указывает, разделено ли поле на разнесенные позиции. |
...
| [Resolution](../../aspose.pdf.forms/barcodefield/resolution/) { get; } | Получает разрешение, в точках на дюйм (dpi), при котором объект штрих-кода отображается. |
| [Scrollable](../../aspose.pdf.forms/textboxfield/scrollable/) { get; set; } | Получает или устанавливает флаг прокручиваемости поля. Если true, поле может быть прокручено. |
...
| [Symbology](../../aspose.pdf.forms/barcodefield/symbology/) { get; } | Указывает, какая технология штрих-кода или глифа будет использоваться в этой аннотации, см. [`Symbology`](./symbology/) для получения подробной информации. |
| [SyncRoot](../../aspose.pdf.forms/field/syncroot/) { get; } | Объект синхронизации. |
| [TabOrder](../../aspose.pdf.forms/field/taborder/) { get; set; } | Получает или устанавливает порядок вкладок поля. |
...
| [XSymHeight](../../aspose.pdf.forms/barcodefield/xsymheight/) { get; } | Получает вертикальное расстояние между двумя модулями штрих-кода, измеренное в пикселях. Соотношение XSymHeight/XSymWidth должно быть целым числом. Для PDF417 допустимый диапазон соотношений составляет от 1 до 4. Для QRCode и DataMatrix это соотношение всегда должно быть 1 |
...
| [ZIndex](../../aspose.pdf/baseparagraph/zindex/) { get; set; } | Получает или устанавливает целочисленное значение, которое указывает порядок Z графика. График с большим ZIndex будет размещен поверх графика с меньшим ZIndex. ZIndex может быть отрицательным. График с отрицательным ZIndex будет размещен за текстом на странице. |
...
| [ExportValueToJson](../../aspose.pdf.forms/field/exportvaluetojson/)(Stream, bool) | Экспортирует содержимое указанного поля в JSON поток. Значения полей кнопок не экспортируются. |
...
| [ImportValueFromJson](../../aspose.pdf.forms/field/importvaluefromjson/)(Stream) | Импортирует данные в указанные поля из JSON потока, основываясь на точном совпадении полных имен полей. |
| [ImportValueFromJson](../../aspose.pdf.forms/field/importvaluefromjson/)(Stream, string) | Импортирует данные в указанное поле из JSON потока, используя полное имя, указанное в переменной 'fieldFullNameInJSON' для сопоставления. |
