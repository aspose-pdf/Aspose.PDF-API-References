---
title: "PDF3DLightingScheme.PDF3DLightingScheme"
second_title: "Справочник API Aspose.PDF для .NET"
description: "Конструктор PDF3DLightingScheme. Инициализирует новый экземпляр класса PDF3DLightingScheme"
type: docs
weight: 10
url: /ru/net/aspose.pdf.annotations/pdf3dlightingscheme/pdf3dlightingscheme/
---
## PDF3DLightingScheme(LightingSchemeType) {#constructor}

Инициализирует новый экземпляр класса [`PDF3DLightingScheme`](../).

```csharp
public PDF3DLightingScheme(LightingSchemeType type)
```

| Параметр | Тип | Описание |
| --- | --- | --- |
| type | LightingSchemeType | Тип схемы освещения. |

### См. также

* enum [LightingSchemeType](../../lightingschemetype/)
* class [PDF3DLightingScheme](../)
* namespace [Aspose.Pdf.Annotations](../../../aspose.pdf.annotations/)
* assembly [Aspose.PDF](../../../)

---

## PDF3DLightingScheme(string) {#constructor_1}

Инициализирует новый экземпляр класса [`PDF3DLightingScheme`](../).

```csharp
public PDF3DLightingScheme(string typeName)
```

| Параметр | Тип | Описание |
| --- | --- | --- |
| typeName | String | Имя типа схемы освещения. |

### Исключения

| исключение | условие |
| --- | --- |
| ArgumentException | Неизвестный аргумент типа схемы освещения |

### См. также

* class [PDF3DLightingScheme](../)
* namespace [Aspose.Pdf.Annotations](../../../aspose.pdf.annotations/)
* assembly [Aspose.PDF](../../../)


