...
| Имя | Описание |
| --- | --- |
| [Flags](../../aspose.pdf.annotations/submitformaction/flags) { get; set; } | Получает или устанавливает флаги действия отправки |
| [Next](../../aspose.pdf.annotations/pdfaction/next) { get; } | Следующие действия по порядку. |
| [Url](../../aspose.pdf.annotations/submitformaction/url) { get; set; } | Целевой URL. |

## Поля

| Имя | Описание |
| --- | --- |
| const [CanonicalFormat](../../aspose.pdf.annotations/submitformaction/canonicalformat) | Если установлено, любые представленные значения полей, представляющие даты, должны быть преобразованы в стандартный формат. |
| const [EmbedForm](../../aspose.pdf.annotations/submitformaction/embedform) | Если установлено, запись F представленного FDF должна быть спецификацией файла, содержащей встроенный файловый поток, представляющий файл PDF, из которого FDF подается. |
...
| const [Exclude](../../aspose.pdf.annotations/submitformaction/exclude) | Если этот параметр не установлен, массив Fields указывает, какие поля включать в отправку. |
...
| const [GetMethod](../../aspose.pdf.annotations/submitformaction/getmethod) | Если установлено, имена полей и значения должны быть представлены с использованием запроса HTTP GET. |
...
| const [SubmitPdf](../../aspose.pdf.annotations/submitformaction/submitpdf) | Если установлено, документ должен быть представлен в формате PDF с использованием типа содержимого MIME application/pdf. |
| const [Xfdf](../../aspose.pdf.annotations/submitformaction/xfdf) | Если установлено, имена полей и значения должны быть представлены в формате XFDF. |
