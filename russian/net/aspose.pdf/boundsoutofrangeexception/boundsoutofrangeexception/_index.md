---
title: "BoundsOutOfRangeException.BoundsOutOfRangeException"
second_title: "Справочник API Aspose.PDF для .NET"
description: "Конструктор BoundsOutOfRangeException. Инициализирует новый экземпляр класса BoundsOutOfRangeException."
type: docs
weight: 10
url: /ru/net/aspose.pdf/boundsoutofrangeexception/boundsoutofrangeexception/
---
## BoundsOutOfRangeException() {#constructor}

Инициализирует новый экземпляр класса [`BoundsOutOfRangeException`](../).

```csharp
public BoundsOutOfRangeException()
```

### См. также

* class [BoundsOutOfRangeException](../)
* namespace [Aspose.Pdf](../../../aspose.pdf/)
* assembly [Aspose.PDF](../../../)

---

## BoundsOutOfRangeException(string) {#constructor_1}

Инициализирует новый экземпляр класса [`BoundsOutOfRangeException`](../) с указанным сообщением об ошибке.

```csharp
public BoundsOutOfRangeException(string message)
```

| Параметр | Тип | Описание |
| --- | --- | --- |
| message | String | Сообщение об ошибке, объясняющее причину исключения. |

### См. также

* class [BoundsOutOfRangeException](../)
* namespace [Aspose.Pdf](../../../aspose.pdf/)
* assembly [Aspose.PDF](../../../)

---

## BoundsOutOfRangeException(string, double, double) {#constructor_2}

Инициализирует новый экземпляр класса [`BoundsOutOfRangeException`](../) с указанным сообщением об ошибке и размерами элемента.

```csharp
public BoundsOutOfRangeException(string message, double containerWidth, double containerHeight)
```

| Параметр | Тип | Описание |
| --- | --- | --- |
| message | String | Сообщение об ошибке, объясняющее причину исключения. |
| containerWidth | Double | Ширина контейнера. |
| containerHeight | Double | Высота контейнера. |

### См. также

* class [BoundsOutOfRangeException](../)
* namespace [Aspose.Pdf](../../../aspose.pdf/)
* assembly [Aspose.PDF](../../../)


