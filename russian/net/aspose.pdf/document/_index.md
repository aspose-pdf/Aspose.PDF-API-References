...
| [Document](document#constructor_1)(Stream) | Инициализировать новый экземпляр документа из потока*input*. |
| [Document](document#constructor_6)(string) | Просто запустите документ, используя*filename*. То же, что[`Document`](./document). |
| [Document](document#constructor_3)(Stream, bool) | Инициализировать новый экземпляр документа из потока*input*. |
...
| [Document](document#constructor_4)(Stream, string) | Инициализировать новый экземпляр документа из потока*input*. |
...
| [Document](document#constructor_8)(string, string) | Инициализирует новый экземпляр класса[`Document`](../document)для работы с зашифрованным документом. |
| [Document](document#constructor_5)(Stream, string, bool) | Инициализировать новый экземпляр документа из потока*input*. |
| [Document](document#constructor_9)(string, string, bool) | Инициализирует новый экземпляр класса[`Document`](../document)для работы с зашифрованным документом. |

## Характеристики

| Имя | Описание |
| --- | --- |
| [Actions](../../aspose.pdf/document/actions) { get; } | Получает действия над документом. Это свойство является экземпляром класса DocumentActions, который позволяет получить/установить действия BeforClosing, BeforSaving и т.д. |
| [AllowReusePageContent](../../aspose.pdf/document/allowreusepagecontent) { get; set; } | Позволяет объединять содержимое страниц для оптимизации размера документа. Если используется, то разные, но дублированные страницы могут ссылаться на один и тот же объект содержимого . Обратите внимание, что этот режим может вызвать побочные эффекты, такие как изменение содержимого страницы при изменении другой страницы. |
...
| [EnableObjectUnload](../../aspose.pdf/document/enableobjectunload) { get; set; } | Получить или установить флаг, позволяющий частично выгрузить документ из памяти. Это позволяет уменьшить использование памяти, но может отрицательно сказаться на производительности. |
...
| [FitWindow](../../aspose.pdf/document/fitwindow) { get; set; } | Получает или устанавливает флаг, указывающий, должно ли окно документа быть изменено в размере, чтобы соответствовать первой отображаемой странице. |
...
| [HandleSignatureChange](../../aspose.pdf/document/handlesignaturechange) { get; set; } | Выбросить исключение, если документ сохранится с изменениями и будет иметь подпись |
...
| [HideToolBar](../../aspose.pdf/document/hidetoolbar) { get; set; } | Получает или устанавливает флаг, указывающий, следует ли скрывать панель инструментов, когда документ активен. |
| [HideWindowUI](../../aspose.pdf/document/hidewindowui) { get; set; } | Получает или устанавливает флаг, указывающий, следует ли скрывать элементы пользовательского интерфейса, когда документ активен. |
...
| [IsPdfaCompliant](../../aspose.pdf/document/ispdfacompliant) { get; } | Получает документ, совместимый с pdfa. |
...
| [Metadata](../../aspose.pdf/document/metadata) { get; } | Метаданные документа. (Документ PDF может включать общую информацию, например, название документа, автора, даты создания и изменения. Такая глобальная информация о документе (в отличие от его содержимого или структуры) называется метаданными и предназначен для помощи в каталогизации и поиске документов во внешних базах данных.) |
...
| [Convert](../../aspose.pdf/document/convert#convert_3)(CallBackGetHocr) | Преобразование документа и сохранение ошибок в указанный файл. |
...
| [Convert](../../aspose.pdf/document/convert#convert_4)(Stream, PdfFormat, ConvertErrorAction) | Конвертировать документ и сохранять ошибки в указанный поток. |
| [Convert](../../aspose.pdf/document/convert#convert_6)(string, PdfFormat, ConvertErrorAction) | Преобразование документа и сохранение ошибок в указанный файл. |
| [Convert](../../aspose.pdf/document/convert#convert)(Fixup, Stream, bool, object[]) | Конвертируйте документ, применив Fixup. |
| [Convert](../../aspose.pdf/document/convert#convert_1)(Fixup, string, bool, object[]) | Конвертируйте документ, применив Fixup. |
| [Convert](../../aspose.pdf/document/convert#convert_5)(Stream, PdfFormat, ConvertErrorAction, ConvertTransparencyAction) | Преобразование документа и сохранение ошибок в указанный файл. |
| [Convert](../../aspose.pdf/document/convert#convert_7)(string, PdfFormat, ConvertErrorAction, ConvertTransparencyAction) | Преобразование документа и сохранение ошибок в указанный файл. |
...
| [Encrypt](../../aspose.pdf/document/encrypt#encrypt_1)(string, string, Permissions, CryptoAlgorithm) | Шифрует документ. Позвоните, затем Сохранить, чтобы получить зашифрованную версию документа. |
| [Encrypt](../../aspose.pdf/document/encrypt#encrypt)(string, string, DocumentPrivilege, CryptoAlgorithm, bool) | Шифрует документ. Позвоните, затем Сохранить, чтобы получить зашифрованную версию документа. |
| [Encrypt](../../aspose.pdf/document/encrypt#encrypt_2)(string, string, Permissions, CryptoAlgorithm, bool) | Шифрует документ. Позвоните, затем Сохранить, чтобы получить зашифрованную версию документа. |
...
| [Optimize](../../aspose.pdf/document/optimize)() | Линеаризовать документ, чтобы - как можно быстрее открыть первую страницу; - максимально быстро отображать следующую страницу или переходить по ссылке на следующую страницу; - отображать страницу постепенно по мере ее поступления, когда данные для страницы доставляются по медленному каналу (сначала отображать наиболее полезные данные); - разрешить взаимодействие с пользователем, например переход по ссылке, даже до того, как вся страница будет получена и отображена. Вызов этого метода фактически не сохраняет документ. Напротив, документ подготовлен только для того, чтобы иметь оптимизированную структуру, вызовите затем Сохранить, чтобы получить оптимизированный документ. |
| [OptimizeResources](../../aspose.pdf/document/optimizeresources#optimizeresources)() | Оптимизировать ресурсы в документе: 1. Удалены ресурсы, которые не используются на страницах документа; 2. Равные ресурсы объединяются в один объект; 3. Неиспользуемые объекты удаляются. |
| [OptimizeResources](../../aspose.pdf/document/optimizeresources#optimizeresources_1)(OptimizationOptions) | Оптимизировать ресурсы в документе в соответствии с заданной стратегией оптимизации. |
| [ProcessParagraphs](../../aspose.pdf/document/processparagraphs)() | Обработка абзацев для генератора. |
...
| [Save](../../aspose.pdf/document/save#save)() | Сохранить документ инкрементно (т. е. используя технику инкрементного обновления). |
...
| [SetTitle](../../aspose.pdf/document/settitle)(string) | Установить заголовок для документа Pdf |
| [SetXmpMetadata](../../aspose.pdf/document/setxmpmetadata)(Stream) | Установить метаданные XMP документа. |
| [Validate](../../aspose.pdf/document/validate#validate)(PdfFormatConversionOptions) | Подтвердить соответствие документа указанному файлу. |
| [Validate](../../aspose.pdf/document/validate#validate_1)(Stream, PdfFormat) | Подтвердить соответствие документа указанному файлу. |
| [Validate](../../aspose.pdf/document/validate#validate_2)(string, PdfFormat) | Подтвердить соответствие документа указанному файлу. |
...
| delegate [FontSubstitutionHandler](document.fontsubstitutionhandler) | Представляет метод, обрабатывающий событие FontSubstitution. |
| interface [IDocumentFontUtilities](document.idocumentfontutilities) | Содержит функциональность для настройки шрифтов |
