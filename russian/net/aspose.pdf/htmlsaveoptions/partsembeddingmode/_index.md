---
title: HtmlSaveOptions.PartsEmbeddingMode
second_title: Aspose.PDF for .NET API Reference
description: Поле HtmlSaveOptions. Оно определяет, будут ли ссылочные файлы встроены в основной HTML файл или будут сгенерированы как отдельные бинарные сущности
type: docs
weight: 390
url: /ru/net/aspose.pdf/htmlsaveoptions/partsembeddingmode/
---
## Поле HtmlSaveOptions.PartsEmbeddingMode

Оно определяет, будут ли ссылочные файлы (HTML, шрифты, изображения, CSS) встроены в основной HTML файл или будут сгенерированы как отдельные бинарные сущности

```csharp
public PartsEmbeddingModes PartsEmbeddingMode;
```

### См. также

* enum [PartsEmbeddingModes](../../htmlsaveoptions.partsembeddingmodes/)
* class [HtmlSaveOptions](../)
* namespace [Aspose.Pdf](../../../aspose.pdf/)
* assembly [Aspose.PDF](../../../)