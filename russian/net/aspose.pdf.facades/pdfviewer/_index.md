...
| Имя | Описание |
| --- | --- |
| [AutoResize](../../aspose.pdf.facades/pdfviewer/autoresize) { get; set; } | Получает или задает логическое значение, указывающее, следует ли печатать файл с оптимизированным размером.  Если false печатать страницу без масштабирования страницы. Если true, распечатайте страницу с масштабированием, чтобы соответствовать области печати. |
...
| [CoordinateType](../../aspose.pdf.facades/pdfviewer/coordinatetype) { get; set; } | Получает или задает тип координат страницы (поля Media/Crop). Значение CropBox используется по умолчанию. |
...
| [PrintAsGrayscale](../../aspose.pdf.facades/pdfviewer/printasgrayscale) { get; set; } | Получает или задает логическое значение, указывающее, печатается ли страница в оттенках серого. По умолчанию ложно. |
...
| [PrinterJobName](../../aspose.pdf.facades/pdfviewer/printerjobname) { get; set; } | Получает или устанавливает имя документа в очереди печати при печати документа. Значение по умолчанию — имя файла. |
...
| [PrintStatus](../../aspose.pdf.facades/pdfviewer/printstatus) { get; } | Получает результат задания печати. Если успех, чем ноль; в противном случае объект исключения. |
| [RenderingOptions](../../aspose.pdf.facades/pdfviewer/renderingoptions) { get; set; } | Получает или задает параметры рендеринга. |
| [Resolution](../../aspose.pdf.facades/pdfviewer/resolution) { get; set; } | Получает или устанавливает разрешение при просмотре и печати. Чем выше разрешение, тем ниже скорость. Значение по умолчанию — 150. |
...
| [UseIntermidiateImage](../../aspose.pdf.facades/pdfviewer/useintermidiateimage) { get; set; } | Возвращает/устанавливает использование преобразования страницы pdf в промежуточный файл png при печати в файловом режиме. Используйте его, когда важен размер выходного файла. |
...
| [PrintDocument](../../aspose.pdf.facades/pdfviewer/printdocument)() | Печатает документ Pdf, используя принтер по умолчанию. |
...
| [PrintLargePdf](../../aspose.pdf.facades/pdfviewer/printlargepdf#printlargepdf_4)(string, PageSettings, PrinterSettings) | Открывает и печатает большой файл Pdf с указанными настройками страницы и настройками принтера. Если ваш файл Pdf содержит сотни страниц и более или его размер превышает 3 МБ, рекомендуется использовать этот метод для повышения производительности . |
