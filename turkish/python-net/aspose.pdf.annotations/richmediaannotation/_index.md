...
description: "PDF belgesine video/ses verisi gömmeye izin veren RichMediaAnnotation'ı tanımlayan sınıf."
type: docs
weight: 710
url: /tr/python-net/aspose.pdf.annotations/richmediaannotation/
---

## RichMediaAnnotation class

PDF belgesine video/ses verisi gömmeye izin veren RichMediaAnnotation'ı tanımlayan sınıf.

RichMediaAnnotation türü aşağıdaki üyeleri sunar:
## Yapıcılar
| Ad | Açıklama |
| :- | :- |
| RichMediaAnnotation(page, rect) | RichMediaAnnotation sınıfının yeni bir örneğini başlatır |
## Özellikler
| Ad | Açıklama |
| :- | :- |
...
| custom_player | Video/ses verilerini oynatmak için özel flash oynatıcıyı ayarlar veya alır. |
| custom_flash_variables | Oynatıcıya geçirilen flash değişkenlerini ayarlar veya alır. |
| içerik | Rich Media içeriğinin verileri. |
| type | İçeriğin türünü alır veya ayarlar. Olası değerler: Audio, Video. |
| activate_on | Uygulamayı etkinleştiren olay. |
## Yöntemler
| Ad | Açıklama |
| :- | :- |
...
| accept(visitor) | Bu açıklama için ziyaretçiyi kabul eder. |
...
| add_custom_data(name, data) | Özel adlandırılmış veri ekler (örneğin flash betiği için gereklidir). |
| set_content(file_name, audio) | İçerik akışını ayarlar. |
| set_poster(image_stream) | Açıklamanın posterini ayarlar. |
| update() | Verileri belirtilen parametrelerle günceller. |
