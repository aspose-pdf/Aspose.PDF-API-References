...
description: "Poligon ek açıklamasını temsil eden sınıf."
type: docs
weight: 650
url: /tr/python-net/aspose.pdf.annotations/polygonannotation/
---

## PolygonAnnotation class

Poligon ek açıklamasını temsil eden sınıf.

PolygonAnnotation türü aşağıdaki üyeleri gösterir:
## Yapıcılar
| Ad | Açıklama |
| :- | :- |
| PolygonAnnotation(document, vertices) | PolygonAnnotation sınıfının yeni bir örneğini başlatır |
| PolygonAnnotation(page, rect, vertices) | PolygonAnnotation sınıfının yeni bir örneğini başlatır |
## Özellikler
| Ad | Açıklama |
| :- | :- |
...
| vertices | Her köşenin yatay ve dikey koordinatlarını temsil eden nokta dizisini alır veya ayarlar. |
| interior_color | Açıklamanın satır sonlarını doldurmak için kullanılacak iç rengi alır veya ayarlar. |
| starting_style | İlk satır sonunun stilini alır veya ayarlar. |
| ending_style | İkinci satır sonunun stilini alır veya ayarlar. |
| intent | Poligon veya çoklu çizgi açıklamasının amacını alır veya ayarlar. |
## Yöntemler
| Ad | Açıklama |
| :- | :- |
| clone() | Yok |
| get_rectangle(consider_rotation) | Yok |
| accept(visitor) | Açıklama işleme için ziyaretçi nesnesini kabul eder. |
| flatten() | Yok |
| change_after_resize(transform) | Matris dönüşümüne göre Vertices içindeki noktaları günceller. |
