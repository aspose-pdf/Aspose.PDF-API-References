---
title: "IllustrationElement"
second_title: "Aspose.PDF for Python via .NET API 参考"
description: "表示逻辑结构中用于插图结构元素的基类。"
type: docs
weight: 250
url: /zh/python-net/aspose.pdf.logicalstructure/illustrationelement/
---

## IllustrationElement class

表示逻辑结构中用于插图结构元素的基类。

IllustrationElement 类型公开以下成员：
## 属性
| 名称 | 描述 |
| :- | :- |
| parent_element | 获取父元素。 |
| child_elements | 获取子集合。 |
| default_attribute_owner | 获取 |
| 属性 | 获取 |
| structure_type | 获取结构元素的类型。 |
| id | 获取结构元素的 ID。 |
| 标题 | 获取或设置结构元素的标题。 |
| language | 获取或设置结构元素的语言。 |
| alternative_text | 获取或设置结构元素的替代文本。 |
| expansion_text | 获取或设置结构元素的展开文本。 |
| actual_text | 获取或设置结构元素的实际文本。 |
## 方法
| 名称 | 描述 |
| :- | :- |
| set_image(image_src, image_resolution) | 将图像追加到当前的 illustration 元素。 |
| set_image(image_src, image_width, image_height) | 将图像追加到当前的 illustration 元素。 |
| append_child(element) | Append |
| change_parent_element(new_parent_element) | 更改当前结构元素的父元素 |
| generate_id() | 为结构元素生成 ID。 |
| set_id(id) | 设置结构元素的 ID。 |
| clear_id() | 清除结构元素的 ID。 |
| set_tag(new_tag) | 为结构元素设置自定义标签。 |

### 另请参阅

* namespace [aspose.pdf.logicalstructure](/pdf/python-net/aspose.pdf.logicalstructure/)
* assembly [Aspose.PDF](/pdf/python-net/)

