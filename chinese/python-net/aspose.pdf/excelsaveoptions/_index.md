...
type: docs
weight: 330
url: /zh/python-net/aspose.pdf/excelsaveoptions/
---
...
ExcelSaveOptions 类型公开以下成员：
## 构造函数
| 名称 | 描述 |
| :- | :- |
| ExcelSaveOptions() | 初始化 ExcelSaveOptions 类的新实例 |
## 属性
| 名称 | 描述 |
| :- | :- |
...
| minimize_the_number_of_worksheets | 如果需要在生成的工作簿中最小化工作表数量，请设为 true。<br/>            默认值为 false；这表示每个 PDF 页面将保存为单独的工作表。 |
| insert_blank_column_at_first | 如果需要在工作表的第一列插入空白列，请设为 true。<br/>            默认值为 false；这表示不会插入空白列。 |
| uniform_worksheets | 如果要在整个文档中使用统一的列划分，请设为 true。 <br/>            默认值为 false；这表示列划分将在每页独立。 |
| format | 输出格式 |
