...
description: "表示用于在弹出窗口中显示文本以进行输入和编辑的弹出注释。"
type: docs
weight: 670
url: /zh/python-net/aspose.pdf.annotations/popupannotation/
---

## PopupAnnotation class

表示用于在弹出窗口中显示文本以进行输入和编辑的弹出注释。

PopupAnnotation 类型公开以下成员：
## 构造函数
| 名称 | 描述 |
| :- | :- |
| PopupAnnotation(document) | 初始化 PopupAnnotation 类的新实例 |
| PopupAnnotation(page, rect) | 初始化 PopupAnnotation 类的新实例 |
## 属性
| 名称 | 描述 |
| :- | :- |
...
| 外观 | 获取该注释的外观字典。 |
| 页面索引 | 获取包含注释的页面索引。 |
| open | 获取或设置一个标志，指定弹出注释是否应初始显示为打开状态。 |
| 父级 | 获取或设置此弹出注释应关联的父注释。<br/>            如果存在此条目，则父注释的 Contents、M、C 和 T 条目应覆盖弹出注释本身的对应条目。 |
## 方法
| 名称 | 描述 |
| :- | :- |
