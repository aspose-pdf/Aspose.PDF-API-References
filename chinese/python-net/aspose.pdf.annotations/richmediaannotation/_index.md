...
description: "描述 RichMediaAnnotation 的类，该注释允许将视频/音频数据嵌入 PDF 文档。"
type: docs
weight: 710
url: /zh/python-net/aspose.pdf.annotations/richmediaannotation/
---

## RichMediaAnnotation class

描述 RichMediaAnnotation 的类，该注释允许将视频/音频数据嵌入 PDF 文档。

RichMediaAnnotation 类型公开以下成员：
## 构造函数
| 名称 | 描述 |
| :- | :- |
| RichMediaAnnotation(page, rect) | 初始化 RichMediaAnnotation 类的新实例 |
## 属性
| 名称 | 描述 |
| :- | :- |
...
| 外观 | 获取该注释的外观字典。 |
| 页面索引 | 获取包含注释的页面索引。 |
| custom_player | 设置或获取用于播放视频/音频数据的自定义 Flash 播放器。 |
| custom_flash_variables | 设置或获取传递给播放器的 Flash 变量。 |
| content | Rich Media 内容的数据。 |
| type | 获取或设置内容类型。可能的值：Audio，Video。 |
| activate_on | 激活应用程序的事件。 |
## 方法
| 名称 | 描述 |
| :- | :- |
| clone() | 克隆此实例。<br/>            虚拟方法。始终返回 null。 |
| get_rectangle(consider_rotation) | 返回考虑页面旋转后的注释矩形。 |
| accept(visitor) | 接受此注释的访问者。 |
| flatten() | 将注释内容直接放置在页面上，<br/>            注释对象将被移除。 |
| change_after_resize(transform) | 根据矩阵变换更新参数和外观。 |
| add_custom_data(name, data) | 添加自定义命名数据（例如 flash 脚本所需）。 |
| set_content(file_name, audio) | 设置内容流。 |
| set_poster(image_stream) | 设置注释的海报。 |
| update() | 使用指定参数更新数据。 |
