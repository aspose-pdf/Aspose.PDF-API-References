...
description: 表示具有通常由调用方提供的抽象方法的类，并处理来自连接的进度事件。
type: docs
weight: 11
url: /zh/java/com.aspose.pdf.facades/pdffileeditor.concatenationprogresshandler/
---
**遗产：**
...
表示具有通常由调用方提供的抽象方法的类，并处理来自连接的进度事件。通常，此类提供的客户处理程序可用于在控制台或进度条上显示总串联进度。
