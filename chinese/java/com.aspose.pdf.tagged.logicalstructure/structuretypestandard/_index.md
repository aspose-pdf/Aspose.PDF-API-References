...
| 场地 | 描述 |
| --- | --- |
| [Annot](#Annot) | （注释；PDF 1.5）部分 ILSE 内容与对应的 PDF 注释之间的关联。 |
| [Art](#Art) | （文章）构成单一叙述或说明的相对独立的文本主体。 |
| [BibEntry](#BibEntry) | （参考书目条目）标识某些引用内容的外部来源的参考。 |
| [BlockQuote](#BlockQuote) | （块引用）文本的一部分，由一个或多个段落组成，这些段落归因于周围文本作者以外的其他人。 |
| [Caption](#Caption) | （标题）描述表格或图形的简短文本部分。 |
| [Code](#Code) | （代码）计算机程序文本的片段。 |
| [Div](#Div) | （划分）通用块级元素或元素组。 |
| [Document](#Document) | (文件)完整的文件。 |
| [Figure](#Figure) | (图) 一项图形内容。 |
| [Form](#Form) | （表单）表示交互式表单字段的小部件注释。 |
| [Formula](#Formula) | （公式）数学公式。 |
| [H](#H) | （标题）文档内容细分的标签。 |
...
| [LBody](#LBody) | (List body) 列表项的描述性内容。 |
| [LI](#LI) | （列表项）列表的单个成员。 |
| [Lbl](#Lbl) | （标签）将给定项目与同一列表或其他类似项目组中的其他项目区分开来的名称或编号。 |
| [Link](#Link) | （链接）部分 ILSE 内容与相应的链接注释或注释之间的关联。 |
| [NonStruct](#NonStruct) | （非结构元素）没有内在结构意义的分组元素；它仅用于分组目的。 |
| [Note](#Note) | （注释）从文档正文中引用的解释性文本项，例如脚注或尾注。 |
| [P](#P) | （段落）文本的低级划分。 |
| [Part](#Part) | （部分）文件的大规模划分。 |
| [Private](#Private) | （私有元素）包含属于生成它的应用程序的私有内容的分组元素。 |
| [Quote](#Quote) | （引用）归因于周围文本作者以外的其他人的文本的内嵌部分。 |
| [RB](#RB) | （Ruby 基本文本）应用了 ruby 注释的全尺寸文本。 |
| [RP](#RP) | （红宝石标点符号）红宝石注释文本周围的标点符号。 |
| [RT](#RT) | （红宝石注释文本）应与红宝石基本文本相邻放置的较小尺寸的文本。 |
| [Reference](#Reference) | （参考）对文档其他地方内容的引用。 |
| [Ruby](#Ruby) | （Ruby；PDF 1.5）以较小的文本大小编写的旁注（注释），并放置在它所指的基本文本旁边。 |
| [Sect](#Sect) | （部分）用于对相关内容元素进行分组的容器。 |
| [Span](#Span) | （跨度）文本的一般内联部分，没有特定的固有特征。 |
| [TBody](#TBody) | （表格主体行组；PDF 1.5）构成表格主体部分的一组行。 |
| [TD](#TD) | （表格数据单元格）包含作为表格内容一部分的数据的表格单元格。 |
| [TFoot](#TFoot) | （表脚行组；PDF 1.5）构成表脚的一组行。 |
| [TH](#TH) | （表格标题单元格）包含标题文本的表格单元格，描述表格的一行或多行或列。 |
| [THead](#THead) | （表格标题行组；PDF 1.5）构成表格标题的一组行。 |
| [TOC](#TOC) | （目录）由目录项条目（结构类型 TOCI）和/或其他嵌套目录条目 (TOC) 组成的列表。 |
| [TOCI](#TOCI) | （目录项）目录的单个成员。 |
| [TR](#TR) | （表格行）表格中的一行标题或数据。 |
| [Table](#Table) | （表格）矩形数据单元的二维布局，可能具有复杂的子结构。 |
| [WP](#WP) | （Warichu 标点符号）WT 文本周围的标点符号。 |
| [WT](#WT) | （Warichu 文本）warichu 评论的较小文本，格式为两行并放置在周围的 WP 元素之间。 |
| [Warichu](#Warichu) | (Warichu; PDF 1.5) 较小文本大小的评论或注释，在包含文本行的高度内被格式化为两条较小的行，并放置在（内嵌）它所指的基本文本之后。 |
## 方法
...
| [getClass()](#getClass--) |  |
| [getTag()](#getTag--) | 获取 StructureElement 的标签名称。 |
| [hashCode()](#hashCode--) |  |
| [notify()](#notify--) |  |
| [notifyAll()](#notifyAll--) |  |
| [toString()](#toString--) | 返回表示当前对象的字符串。 |
| [to_StructureTypeStandard(String tag)](#to-StructureTypeStandard-java.lang.String-) | 执行从 String 到[StructureTypeStandard](../../com.aspose.pdf.tagged.logicalstructure/structuretypestandard). |
| [wait()](#wait--) |  |
...
**退货：**
[StructureTypeCategory](../../com.aspose.pdf.tagged.logicalstructure/structuretypecategory) - 值：标准结构类型的类别。
### getClass() {#getClass--}
```
...
**退货：**
java.lang.String - StructureElement 的标记名称。
### hashCode() {#hashCode--}
```
...
执行从 String 到[StructureTypeStandard](../../com.aspose.pdf.tagged.logicalstructure/structuretypestandard).
