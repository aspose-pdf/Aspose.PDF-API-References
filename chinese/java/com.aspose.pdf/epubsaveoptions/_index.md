...
| [getClass()](#getClass--) |  |
| [getContentRecognitionMode()](#getContentRecognitionMode--) | 在转换 PDF 文件（通常具有固定布局）时，转换引擎会尝试执行分组和多级分析以还原原始文档作者的意图并产生流式布局的结果。 |
...
| [setContentRecognitionMode(int contentRecognitionMode)](#setContentRecognitionMode-int-) | 在转换 PDF 文件（通常具有固定布局）时，转换引擎会尝试执行分组和多级分析以还原原始文档作者的意图并产生流式布局的结果。 |
...
| 范围 | 类型 | 描述 |
| --- | --- | --- |
| contentRecognitionMode | int | RecognitionMode 元素 |
