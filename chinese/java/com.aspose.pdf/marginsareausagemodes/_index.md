...
description: 表示转换期间边距区域的使用模式，如 HTML EPUB 等定义了与边距使用相关的导入格式指令的处理。
type: docs
weight: 10
...
表示转换期间边距区域的使用模式（如 HTML、EPUB 等），定义与边距使用相关的导入格式指令的处理。
## 领域

| 场地 | 描述 |
| --- | --- |
| [EnumSeparatorCharArray](#EnumSeparatorCharArray) |  |
| [NeverPutContentOnMarginArea](#NeverPutContentOnMarginArea) | 此模式严格禁止使用边距区域，因此，转换器永远不会使用边距区域进行渲染，即使 CSS 或源文档格式允许或要求 |
| [PutContentOnMarginAreaIfNecessary](#PutContentOnMarginAreaIfNecessary) | 在此模式下，转换器遵循导入文档的格式（fe |
## 方法
...
此模式严格禁止使用边距区域，因此，转换器永远不会使用边距区域进行渲染，即使 CSS 或源文档格式允许或要求
...
在此模式下，转换器在边距区域的使用中遵循导入文档的格式（导入 HTML 的 fe CSS）。因此，如果导入文档的格式需要使用边距区域进行渲染，转换器将允许
