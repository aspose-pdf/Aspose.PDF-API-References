...
description: 表示显示页面的显式目标，其内容放大到足以使其边界框在水平和垂直方向上完全适合窗口。
type: docs
weight: 119
...
表示显示页面的显式目标，其内容放大到足以使其边界框在水平和垂直方向上完全适合窗口。如果所需的水平和垂直放大系数不同，则使用两者中较小的一个，将边界框在另一个维度的窗口内居中。
## 构造函数

| 构造函数 | 描述 |
| --- | --- |
| [FitBExplicitDestination(Page page)](#FitBExplicitDestination-com.aspose.pdf.Page-) | 创建实例并通过 DOM 页面对象对其进行初始化。 |
| [FitBExplicitDestination(Document document, int pageNumber)](#FitBExplicitDestination-com.aspose.pdf.Document-int-) | 创建远程显式目标。 |
| [FitBExplicitDestination(int pageNumber)](#FitBExplicitDestination-int-) | 创建远程显式目标。 |
## 方法
