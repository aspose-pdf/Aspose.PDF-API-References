...
description: 此枚举描述了转换期间可能的抗锯齿措施
type: docs
weight: 10
...
| [NoAdditionalProcessing](#NoAdditionalProcessing) | 没有使用特殊的抗锯齿处理。 |
| [TryCorrectResultHtml](#TryCorrectResultHtml) | 在这种情况下，转换器会尝试检测具有相邻背景图形元素的位置，并以相关方式更正结果 HTML。 |
## 方法
...
没有使用特殊的抗锯齿处理。这是压倒大多数文档的最佳选择，并且在转换过程中不需要额外的时间
...
在这种情况下，转换器会尝试检测具有相邻背景图形元素的位置，并以相关方式更正结果 HTML。此选项允许增强包含由多个相邻图形元素构建的背景的文档的导出结果（对于此类文档，PDF 渲染器，fe Acrobat Reader，通常在渲染期间尝试平滑元素边界。使用此选项转换器模仿 PDF-渲染器。此选项允许为某些特定文档（使用此类复合背景）增强导出结果的布局，但它需要额外的处理时间（通常约为额外时间的 10-15%）。因此在一般情况下使用此模式不推荐。
