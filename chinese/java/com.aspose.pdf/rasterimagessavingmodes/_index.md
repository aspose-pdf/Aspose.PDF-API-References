...
description: 转换后的 PDF 可以包含光栅图像 .png .jpeg 等。
type: docs
weight: 24
...
| 场地 | 描述 |
| --- | --- |
| [AsEmbeddedPartsOfPngPageBackground](#AsEmbeddedPartsOfPngPageBackground) | 将为每个结果页面生成一个大的 PNG 背景文件。 |
| [AsExternalPngFilesReferencedViaSvg](#AsExternalPngFilesReferencedViaSvg) | 不同的光栅图像将被分开为 PNG 文件，但将通过包装 SVG 图像进行引用，即将为每个光栅图像生成一个 PNG 文件和一个 SVG，并且每个这样的 SVG 将包含指向相关 PNG 文件的链接 |
| [AsPngImagesEmbeddedIntoSvg](#AsPngImagesEmbeddedIntoSvg) | 对于每个不同的光栅文件，将生成包装 SVG 图像，并且光栅图像将作为 Base64 编码字符串嵌入到该 SVG 图像中 |
...
将为每个结果页面生成一个大的 PNG 背景文件。光栅图像将嵌入到该文件中并呈现为该图像的区域。不会为每个图像生成外部 PNG 文件，每页只有一个 PNG 文件将出现在文件转换结果集中。
...
不同的光栅图像将被分开为 PNG 文件，但将通过包装 SVG 图像进行引用，即将为每个光栅图像生成一个 PNG 文件和一个 SVG，并且每个这样的 SVG 将包含指向相关 PNG 文件的链接
...
对于每个不同的光栅文件，将生成包装 SVG 图像，并且光栅图像将作为 Base64 编码字符串嵌入到该 SVG 图像中
