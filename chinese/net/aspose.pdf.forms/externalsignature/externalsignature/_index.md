...
description: "ExternalSignature 构造函数。使用 X509Certificate2 创建一个分离的 PKCS7 分离签名。它支持不具备可导出私钥的 USB 智能卡令牌。"
type: docs
weight: 10
...
使用 X509Certificate2 创建分离的 PKCS#7 `(detached)` 签名。它支持没有可导出私钥的 USB 智能卡、令牌。
...
| --- | --- | --- |
| 证书 | X509Certificate2 | 带有私钥的证书。 |
| 分离 | Boolean | 如果签名应为分离，则为 True，否则为 false。 |

## 备注

当 detached 设置为 false 时，摘要算法将始终为 `SHA1`。否则，摘要算法将根据证书密钥数据自动选择（参见 Auto）。
...
| 参数 | 类型 | 描述 |
| --- | --- | --- |
| base64 | String | X509Certificate2 的 base64 字符串。 |
| 分离 | Boolean | 如果签名应为分离，则为 True，否则为 false。 |
...
使用 X509Certificate2 作为 Base64 字符串创建 PKCS#7 `(detached)` 签名。
