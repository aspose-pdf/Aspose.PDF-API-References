...
description: "Aspose.Pdf.Devices.GifDevice 类。表示帮助将 pdf 文档页面保存为 gif 的图像设备。"
type: docs
weight: 3720
...
| [GifDevice](gifdevice/#constructor_2)(PageSize) | 使用提供的页面尺寸和默认分辨率（=150）初始化 `GifDevice` 类的新实例。 |
| [GifDevice](gifdevice/#constructor_1)(Resolution) | 初始化 `GifDevice` 类的新实例。结果图像文件的分辨率，请参阅 [`Resolution`](../resolution/) 类。 |
| [GifDevice](gifdevice/#constructor_4)(int, int) | 使用提供的图像尺寸和默认分辨率（=150）初始化 `GifDevice` 类的新实例。 |
| [GifDevice](gifdevice/#constructor_3)(PageSize, Resolution) | 使用提供的页面尺寸和分辨率初始化 `GifDevice` 类的新实例。 |
...
| override [Process](../../aspose.pdf.devices/gifdevice/process/#process)(Page, Stream) | 将页面转换为 gif 并保存到输出流中。 |
| [Process](../../aspose.pdf.devices/pagedevice/process/)(Page, string) | 对给定页面执行某些操作并将结果保存到文件中。 |

## 示例

以下示例展示了如何将 PDF 文件转换为 GIF 图像。
