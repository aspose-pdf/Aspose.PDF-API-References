...
description: "Aspose.Pdf.Devices.BmpDevice 类。表示帮助将 pdf 文档页面保存为 bmp 的图像设备。"
type: docs
weight: 3640
...
| [BmpDevice](bmpdevice/#constructor_2)(PageSize) | 使用提供的页面尺寸和默认分辨率（=150）初始化 `BmpDevice` 类的新实例。 |
| [BmpDevice](bmpdevice/#constructor_1)(Resolution) | 初始化 `BmpDevice` 类的新实例。结果图像文件的分辨率，请参阅 [`Resolution`](../resolution/) 类。 |
| [BmpDevice](bmpdevice/#constructor_4)(int, int) | 使用提供的图像尺寸和默认分辨率（=150）初始化 `BmpDevice` 类的新实例。 |
| [BmpDevice](bmpdevice/#constructor_3)(PageSize, Resolution) | 使用提供的页面尺寸和分辨率初始化 `BmpDevice` 类的新实例。 |
...
| override [Process](../../aspose.pdf.devices/bmpdevice/process/#process)(Page, Stream) | 将页面转换为 bmp 并将其保存到输出流中。 |
| [Process](../../aspose.pdf.devices/pagedevice/process/)(Page, string) | 对给定页面执行某些操作并将结果保存到文件中。 |

## 示例

下面的示例展示了如何将 PDF 文件转换为 BMP 图像。
