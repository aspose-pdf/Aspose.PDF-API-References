---
title: "ColorBarAnnotation 类"
second_title: "Aspose.PDF for .NET API 参考"
description: "Aspose.Pdf.Annotations.ColorBarAnnotation 类。表示 ColorBarAnnotation 注释的类。属性 Color 被忽略，改为使用 ColorsOfCMYK 颜色。创建时，宽高比决定注释的方向（水平或垂直）。随后检查注释矩形是否在 TrimBox 之外，如果不在，则根据注释的方向将其移动到最近的 TrimBox 外部位置。可以缩小宽高，使注释适配在 TrimBox 之外。如果布局没有空间，宽度/高度可以设为零，此时注释仍在页面上，但不显示。"
type: docs
weight: 1690
...
表示 ColorBarAnnotation 注释的类。属性 Color 被忽略，改用 ColorsOfCMYK 颜色。创建时，宽高比决定注释的方向——水平或垂直。随后检查注释矩形是否位于 TrimBox 之外，如果不在，则根据注释的方向将其移动到最近的 TrimBox 外部位置。可以缩小宽度（高度），使注释位于 TrimBox 之外。如果布局没有空间，宽度/高度可以设为零（此时注释仍在页面上，但不显示）。
...
| override [AnnotationType](../../aspose.pdf.annotations/colorbarannotation/annotationtype/) { get; } | 获取 Annotation 的类型。 |
...
| [ColorOfCMYK](../../aspose.pdf.annotations/colorbarannotation/colorofcmyk/) { get; set; } | 获取或设置注释绘制使用的颜色（青色、品红、黄色、黑色之一）。 |
...
| 名称 | 描述 |
| --- | --- |
| override [Accept](../../aspose.pdf.annotations/colorbarannotation/accept/)(AnnotationSelector) | 接受访问者对象以处理该注释。 |
| override [ChangeAfterResize](../../aspose.pdf.annotations/colorbarannotation/changeafterresize/)(Matrix) | 根据矩阵变换更新参数和外观，并在必要时将其移动到 TrimBox 之外。 |
