...
description: 表示橡皮图章注解 这种类型的注释显示的文本或图形看起来像是用橡皮图章印在页面上的
type: docs
weight: 1160
...
表示橡皮图章注解。 这种类型的注释显示的文本或图形看起来像是用橡皮图章印在页面上的。
...
| 姓名 | 描述 |
| --- | --- |
| override [Accept](../../aspose.pdf.annotations/stampannotation/accept)(AnnotationSelector) | Acepts[`AnnotationSelector`](../annotationselector)浏览注释集合时的访问者。 |
...
下一个代码片段演示了如何在第一个 pdf 文档页面中添加 2 个图章。 输入文档来自inFile，更改保存到outFile。 第一张邮票带有 NotForPublicRelease 图标，第二张带有来自 Rubber.jpg 的图片。
