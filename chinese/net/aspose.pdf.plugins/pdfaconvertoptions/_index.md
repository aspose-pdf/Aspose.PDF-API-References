...
description: Aspose.Pdf.Plugins.PdfAConvertOptions class. 表示将 PDF 文档转换为 PDF/A 格式的选项，使用 PdfAConverter 插件
type: docs
weight: 8990
...
表示将 PDF 文档转换为 PDF/A 格式的选项，使用 [`PdfAConverter`](../pdfaconverter/) 插件。
...
| [Outputs](../../aspose.pdf.plugins/pdfaconvertoptions/outputs/) { get; } | 获取添加的目标（文件或流数据源）集合，用于保存操作结果。 |
| [PdfAVersion](../../aspose.pdf.plugins/pdfaoptionsbase/pdfaversion/) { get; set; } | 获取或设置用于验证或转换的 PDF/A 标准版本。 |
| [PuaSymbolsProcessingStrategy](../../aspose.pdf.plugins/pdfaoptionsbase/puasymbolsprocessingstrategy/) { get; set; } | 获取或设置处理 PDF 文档中私有使用区（PUA）符号的策略。 |
| [SoftMaskAction](../../aspose.pdf.plugins/pdfaoptionsbase/softmaskaction/) { get; set; } | 获取或设置在转换带有软遮罩的图像时所采取的操作。 |
| [SymbolicFontEncodingStrategy](../../aspose.pdf.plugins/pdfaoptionsbase/symbolicfontencodingstrategy/) { get; set; } | 获取或设置在转换为 PDF/A 格式时编码符号字体的策略。 |
