...
description: Aspose.Pdf.FooterArtifact 类。描述页脚工件。这可以用于设置页面的页脚
type: docs
weight: 4930
...
| [BottomMargin](../../aspose.pdf/artifact/bottommargin/) { get; set; } | 工件的下边距。如果位置在 Position 属性中明确指定，则此值将被忽略。 |
...
| [TopMargin](../../aspose.pdf/artifact/topmargin/) { get; set; } | 工件的上边距。如果位置在 Position 属性中明确指定，则此值将被忽略。 |
...
| Name | Description |
| --- | --- |
| [BeginUpdates](../../aspose.pdf/artifact/beginupdates/)() | 开始延迟更新。如果您需要对同一工件进行多次更改以提高性能，请使用此功能。通常，当工件属性发生更改时，工件操作符会随之更改。这会导致每次更改工件时页面内容都会发生变化。为避免这种效果，请将所有工件更新放在 StartUpdates/SaveUpdates 调用之间。这允许仅更改一次页面内容。 |
