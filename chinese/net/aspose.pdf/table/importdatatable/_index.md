...
description: "Table 方法。将数据从 System.Data.DataTable 导入到 Aspose.Pdf.Table"
type: docs
weight: 260
...
| isColumnNamesImported | Boolean | 指定是否将列名导入为第一行 |
| firstFilledRow | Int32 | 指定目标表中导入开始的第一行的零基编号，如果目标表中不存在该编号的行（以及之前的某些行），将首先创建它们 |
...
| isColumnNamesShown | Boolean | 指定是否将源数据表的列名导入为第一行。 |
| firstFilledRow | Int32 | 指定目标表中导入开始的第一行的零基编号，如果目标表中不存在该编号的行（以及之前的某些行），将首先创建它们 |
| firstFilledColumn | Byte | 指定目标表中第一目标列的编号，列必须在导入开始前已存在于目标表中 |
| maxRows | Int32 | 从源表导入的最大行数。 |
...
将 DataTable 对象导入，但不是作为整体实体。仅导入指定的行和列。
...
| --- | --- | --- |
| importedDataTable | DataTable | 要导入的 DataTable 对象。 |
| sourceRowList | Int32[] | 必须导入的源 DataTable 对象中行号的数组。列表不能为空，并且只能包含现有行的编号，否则将抛出异常。 |
| sourceColumnList | Int32[] | 必须导入的源 DataTable 对象中列号的数组。列表不能为空，并且只能包含现有列的编号，否则将抛出异常。 |
| firstFilledRow | Int32 | 导入开始的 targer 表中第一个单元格的零基行号。如果目标表不包含该行，则会创建该行（以及必要的所有前置行）。 |
| firstFilledColumn | Int32 | 导入开始的 targer 表中第一个单元格的零基列号。目标表必须在导入开始前包含该列，否则将抛出异常。 |
| showColumnNamesAsFirstRow | Boolean | 指定是否将源数据表的列名导入为第一行。 |
| isHtmlSupported | Boolean | 指定文本是否为 HTML 字符串。 |
