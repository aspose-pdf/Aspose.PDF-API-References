---
title: "HtmlLoadOptions.CustomLoaderOfExternalResources"
second_title: "Aspose.PDF for .NET API 参考"
description: "HtmlLoadOptions 字段。有时需要避免使用内部加载器来加载外部资源（如图像或 CSS），并提供自定义方法从其他位置获取请求的资源。例如，在云环境中使用 Aspose.PDF 时，无法直接访问引用的文件，此时应将自定义代码放入专用方法，并将指向该方法的委托分配给此属性。"
type: docs
weight: 100
url: /zh/net/aspose.pdf/htmlloadoptions/customloaderofexternalresources/
---
## HtmlLoadOptions.CustomLoaderOfExternalResources field

有时需要避免使用内部加载器来加载外部资源（如图像或 CSS），并提供自定义方法以从某处获取请求的资源。例如，在云环境中使用 Aspose.PDF 时，无法直接访问引用的文件：此情况下应使用放入特定方法的自定义代码，并将指向该方法的委托分配给此属性。

```csharp
public ResourceLoadingStrategy CustomLoaderOfExternalResources;
```

### 另请参见

* delegate [ResourceLoadingStrategy](../../loadoptions.resourceloadingstrategy/)
* class [HtmlLoadOptions](../)
* namespace [Aspose.Pdf](../../../aspose.pdf/)
* assembly [Aspose.PDF](../../../)


