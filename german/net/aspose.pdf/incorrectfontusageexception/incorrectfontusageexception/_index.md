---
title: IncorrectFontUsageException
second_title: Aspose.PDF für .NET-API-Referenz
description: Initialisiert eine neue Instanz vonIncorrectFontUsageExceptionaspose.pdf/incorrectfontusageexception Klasse.
type: docs
weight: 10
url: /de/net/aspose.pdf/incorrectfontusageexception/incorrectfontusageexception/
---
## IncorrectFontUsageException(string) {#constructor_1}

Initialisiert eine neue Instanz von[`IncorrectFontUsageException`](../../incorrectfontusageexception) Klasse.

```csharp
public IncorrectFontUsageException(string message)
```

| Parameter | Typ | Beschreibung |
| --- | --- | --- |
| message | String | Die Nachricht. |

### Siehe auch

* class [IncorrectFontUsageException](../../incorrectfontusageexception)
* namensraum [Aspose.Pdf](../../incorrectfontusageexception)
* Montage [Aspose.PDF](../../../)

---

## IncorrectFontUsageException(string, Exception) {#constructor_2}

Initialisiert eine neue Instanz von[`IncorrectFontUsageException`](../../incorrectfontusageexception) Klasse mit einer angegebenen Fehlermeldung und einem Verweis auf die innere Ausnahme, die die Ursache dieser Ausnahme ist.

```csharp
public IncorrectFontUsageException(string message, Exception innerException)
```

| Parameter | Typ | Beschreibung |
| --- | --- | --- |
| message | String | Die Fehlermeldung, die den Grund für die Ausnahme erklärt. |
| innerException | Exception | Die Ausnahme, die die Ursache der aktuellen Ausnahme ist, oder ein NULL-Verweis (Nothing in Visual Basic), wenn keine innere Ausnahme angegeben ist. |

### Siehe auch

* class [IncorrectFontUsageException](../../incorrectfontusageexception)
* namensraum [Aspose.Pdf](../../incorrectfontusageexception)
* Montage [Aspose.PDF](../../../)

---

## IncorrectFontUsageException(Exception) {#constructor}

Initialisiert eine neue Instanz von[`IncorrectFontUsageException`](../../incorrectfontusageexception) Klasse mit einer angegebenen Fehlermeldung und einem Verweis auf die innere Ausnahme, die die Ursache dieser Ausnahme ist.

```csharp
public IncorrectFontUsageException(Exception innerException)
```

| Parameter | Typ | Beschreibung |
| --- | --- | --- |
| innerException | Exception | Die Ausnahme, die die Ursache der aktuellen Ausnahme ist, oder ein NULL-Verweis (Nothing in Visual Basic), wenn keine innere Ausnahme angegeben ist. |

### Siehe auch

* class [IncorrectFontUsageException](../../incorrectfontusageexception)
* namensraum [Aspose.Pdf](../../incorrectfontusageexception)
* Montage [Aspose.PDF](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.PDF.dll -->
