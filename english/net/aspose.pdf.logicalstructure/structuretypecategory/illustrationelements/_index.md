---
title: IllustrationElements
second_title: Aspose.PDF for .NET API Reference
description: Illustration elements are compact sequences of content in page content order that are considered to be unitary objects with respect to page layout. An illustration can be treated as either a BLSE or an ILSE.
type: docs
weight: 30
url: /net/aspose.pdf.logicalstructure/structuretypecategory/illustrationelements/
---
## StructureTypeCategory.IllustrationElements field

Illustration elements are compact sequences of content, in page content order, that are considered to be unitary objects with respect to page layout. An illustration can be treated as either a BLSE or an ILSE.

```csharp
public static readonly StructureTypeCategory IllustrationElements;
```

### See Also

* class [StructureTypeCategory](../../structuretypecategory)
* namespace [Aspose.Pdf.LogicalStructure](../../structuretypecategory)
* assembly [Aspose.PDF](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.PDF.dll -->
