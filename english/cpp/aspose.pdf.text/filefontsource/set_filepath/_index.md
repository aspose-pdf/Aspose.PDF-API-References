---
title: Aspose::Pdf::Text::FileFontSource::set_FilePath method
linktitle: set_FilePath
second_title: Aspose.PDF for C++ API Reference
description: 'Aspose::Pdf::Text::FileFontSource::set_FilePath method. Path to the font file in C++.'
type: docs
weight: 400
url: /cpp/aspose.pdf.text/filefontsource/set_filepath/
---
## FileFontSource::set_FilePath method


Path to the font file.

```cpp
void Aspose::Pdf::Text::FileFontSource::set_FilePath(const System::String &value)
```

## See Also

* Class [String](../../../system/string/)
* Class [FileFontSource](../)
* Namespace [Aspose::Pdf::Text](../../)
* Library [Aspose.PDF for C++](../../../)
