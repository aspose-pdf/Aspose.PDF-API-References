---
title: Aspose::Pdf::HtmlSaveOptions::set_DocumentType method
linktitle: set_DocumentType
second_title: Aspose.PDF for C++ API Reference
description: 'Aspose::Pdf::HtmlSaveOptions::set_DocumentType method. Sets the HtmlDocumentType in C++.'
type: docs
weight: 200
url: /cpp/aspose.pdf/htmlsaveoptions/set_documenttype/
---
## HtmlSaveOptions::set_DocumentType method


Sets the [HtmlDocumentType](../../htmldocumenttype/).

```cpp
ASPOSE_PDF_SHARED_API void Aspose::Pdf::HtmlSaveOptions::set_DocumentType(HtmlDocumentType value)
```

## Remarks


The [HtmlDocumentType](../../htmldocumenttype/). 
## See Also

* Enum [HtmlDocumentType](../../htmldocumenttype/)
* Class [HtmlSaveOptions](../)
* Namespace [Aspose::Pdf](../../)
* Library [Aspose.PDF for C++](../../../)
