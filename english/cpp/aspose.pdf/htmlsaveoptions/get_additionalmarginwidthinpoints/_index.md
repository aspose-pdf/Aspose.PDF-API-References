---
title: Aspose::Pdf::HtmlSaveOptions::get_AdditionalMarginWidthInPoints method
linktitle: get_AdditionalMarginWidthInPoints
second_title: Aspose.PDF for C++ API Reference
description: 'Aspose::Pdf::HtmlSaveOptions::get_AdditionalMarginWidthInPoints method. If attribute ''SplitOnPages=false'', than whole HTML representing all input PDF pages wont be not split into different HTML pages, but will be put into one big result HTML file. But each source PDF page will be represented with it''s own rectangle area in HTML (if necessary that areas can be bordered to show page paper edges with special attribute ''PageBorderIfAny''. This parameter defines width of margin that will be forcibly left around that output HTML-areas that represent pages of source PDF document.In essence it defines guaranteed interval between HTML-representations of PDF "paper" pages such mode of conversion in C++.'
type: docs
weight: 2600
url: /cpp/aspose.pdf/htmlsaveoptions/get_additionalmarginwidthinpoints/
---
## HtmlSaveOptions::get_AdditionalMarginWidthInPoints method


If attribute 'SplitOnPages=false', than whole HTML representing all input PDF pages wont be not split into different HTML pages, but will be put into one big result HTML file. But each source PDF page will be represented with it's own rectangle area in HTML (if necessary that areas can be bordered to show page paper edges with special attribute 'PageBorderIfAny'. This parameter defines width of margin that will be forcibly left around that output HTML-areas that represent pages of source PDF document.In essence it defines guaranteed interval between HTML-representations of PDF "paper" pages such mode of conversion.

```cpp
ASPOSE_PDF_SHARED_API int32_t Aspose::Pdf::HtmlSaveOptions::get_AdditionalMarginWidthInPoints() const
```

## Remarks


<xrefsect id="deprecated_1_deprecated000105">
  <xreftitle>Deprecated</xreftitle>
  <xrefdescription>
    <para>AdditionalMarginWidthInPoints is deprecated, please use PageMarginIfAny instead. </para>
  </xrefdescription>
</xrefsect>
## See Also

* Class [HtmlSaveOptions](../)
* Namespace [Aspose::Pdf](../../)
* Library [Aspose.PDF for C++](../../../)
