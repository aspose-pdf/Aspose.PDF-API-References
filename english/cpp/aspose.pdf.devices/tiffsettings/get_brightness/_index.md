---
title: Aspose::Pdf::Devices::TiffSettings::get_Brightness method
linktitle: get_Brightness
second_title: Aspose.PDF for C++ API Reference
description: 'Aspose::Pdf::Devices::TiffSettings::get_Brightness method. Get or sets a value boundary of the transformation of colors in white and black. This parameter can be applied with EncoderValue.CompressionCCITT4, EncoderValue.CompressionCCITT3, EncoderValue.CompressionRle or ColorDepth.Format1bpp == 1 in C++.'
type: docs
weight: 1000
url: /cpp/aspose.pdf.devices/tiffsettings/get_brightness/
---
## TiffSettings::get_Brightness method


Get or sets a value boundary of the transformation of colors in white and black. This parameter can be applied with EncoderValue.CompressionCCITT4, EncoderValue.CompressionCCITT3, EncoderValue.CompressionRle or [ColorDepth.Format1bpp](../../colordepth/) == 1.

```cpp
ASPOSE_PDF_SHARED_API float Aspose::Pdf::Devices::TiffSettings::get_Brightness() const
```

## Remarks


Value of brightness should be in the range from 0 to 1. By default value is equal to 0.33f
## See Also

* Class [TiffSettings](../)
* Namespace [Aspose::Pdf::Devices](../../)
* Library [Aspose.PDF for C++](../../../)
