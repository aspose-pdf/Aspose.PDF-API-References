---
title: Aspose::Pdf::LogicalStructure::TableElement::set_RepeatingRowsStyle method
linktitle: set_RepeatingRowsStyle
second_title: Aspose.PDF for C++ API Reference
description: 'Aspose::Pdf::LogicalStructure::TableElement::set_RepeatingRowsStyle method. Gets the style for repeating rows in C++.'
type: docs
weight: 3900
url: /cpp/aspose.pdf.logicalstructure/tableelement/set_repeatingrowsstyle/
---
## TableElement::set_RepeatingRowsStyle method


Gets the style for repeating rows.

```cpp
void Aspose::Pdf::LogicalStructure::TableElement::set_RepeatingRowsStyle(const System::SharedPtr<Text::TextState> &value)
```

## See Also

* Typedef [SharedPtr](../../../system/sharedptr/)
* Class [TextState](../../../aspose.pdf.text/textstate/)
* Class [TableElement](../)
* Namespace [Aspose::Pdf::LogicalStructure](../../)
* Library [Aspose.PDF for C++](../../../)
