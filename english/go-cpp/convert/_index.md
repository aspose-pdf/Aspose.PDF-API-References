---
title: "Convert from PDF functions"
second_title: Aspose.PDF for Go via C++
description:  "Functions for converting from PDF files."
type: docs
url: /go-cpp/convert/
---

## Convert from PDF functions

|      Name      |  Description   |
| -------------- | -------------- |
| [SaveDocX](./savedocx/) | Convert and save the previously opened PDF-document as DocX-document. |
| [SaveDoc](./savedoc/) | Convert and save the previously opened PDF-document as Doc-document. |
| [SaveXlsX](./savexlsx/) | Convert and save the previously opened PDF-document as XlsX-document. |
| [SaveTxt](./savetxt/) | Convert and save the previously opened PDF-document as Txt-document. |
| [SavePptX](./savepptx/) | Convert and save the previously opened PDF-document as PptX-document. |
| [SaveXps](./savexps/) | Convert and save the previously opened PDF-document as Xps-document. |
| [SaveTeX](./savetex/) | Convert and save the previously opened PDF-document as TeX-document. |
| [SaveEpub](./saveepub/) | Convert and save the previously opened PDF-document as Epub-document. |
| [PageToJpg](./pagetojpg/) | Convert and save the specified page as Jpg-image. |
| [PageToPng](./pagetopng/) | Convert and save the specified page as Png-image. |
| [PageToBmp](./pagetobmp/) | Convert and save the specified page as Bmp-image. |
| [PageToTiff](./pagetotiff/) | Convert and save the specified page as Tiff-image. |
| [PageToSvg](./pagetosvg/) | Convert and save the specified page as Svg-image. |
| [PageToPdf](./pagetopdf/) | Convert and save the specified page as Pdf. |
| [PageToDICOM](./pagetodicom/) | Convert and save the specified page as DICOM-image. |


## Detailed Description

Functions for converting from PDF files.

