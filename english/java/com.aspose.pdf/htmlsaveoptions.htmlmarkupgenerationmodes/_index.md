...
| Field | Description |
| --- | --- |
| [WriteAllHtml](#WriteAllHtml) | Default mode any specific requirments are absent. Will be generated output that will contain all parts of HTML without any special additional processing. |
| [WriteOnlyBodyContent](#WriteOnlyBodyContent) | will be stripped away all HTML content that is outside HTML's body, i.e. will be left only content that is inside {@code } tags |
...
will be stripped away all HTML content that is outside HTML's body, i.e. will be left only content that is inside {@code } tags
